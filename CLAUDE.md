...
Fünf PR-getriggerte Workflows liegen in [.github/workflows/](.github/workflows/) (kein `push`-Trigger auf `main` — die PR-Gates davor genügen). Auf **jedem** Pull Request laufen drei davon als Gate — `ci.yml`, `pr-labeler.yml`, `dependency-review.yml`; die anderen beiden laufen bedingt: `renovate-validate.yml` nur, wenn die Renovate-Config oder ihr eigener Workflow im Diff liegt, `expo-sdk-sync.yml` nur auf Renovates Expo-SDK-PR — und ist dabei kein Gate, sondern ein mutierender Workflow, der Commits in den PR-Branch pusht:
