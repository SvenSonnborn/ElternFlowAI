...
- **`ci.yml`** — zwei parallele Jobs. `quality` läuft `format:check` · `lint` · `typecheck` · `test` als getrennte Steps (der `test`-Step nutzt `bun test`, Buns Runner — **nicht** `bun run test`/`jest`, das unter jest-expo 55 kaputt ist; siehe Tech-Stack-Notiz zu `bun test`); die nachgelagerten (`lint`/`typecheck`/`test`) tragen `if: ${{ !cancelled() }}`, damit ein früher Fehler die späteren nicht verdeckt; `build` macht `bunx expo export --platform web --output-dir dist` als Smoke-Test. Runtime: Bun `1.3.10` (gepinnt), `bun install --frozen-lockfile`.
