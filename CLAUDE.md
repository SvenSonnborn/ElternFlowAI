...
Fünf PR-getriggerte Workflows in [.github/workflows/](.github/workflows/) sichern jeden Pull Request ab (kein `push`-Trigger auf `main` — die PR-Gates davor genügen):
...
- **`renovate-validate.yml`** — validiert [.github/renovate.json5](.github/renovate.json5) mit dem offiziellen `renovate-config-validator` (`npx --yes --package renovate@latest -- renovate-config-validator --strict --no-global .github/renovate.json5`). Die Versionsfixierung auf `renovate@latest` ist bewusst: ein bare `--package renovate` lässt npx jeden zwischengespeicherten Cache-Treffer wiederverwenden, was lokal während der Umsetzung einen `renovate@37.440.7` von Juli 2024 zog und einen korrekten Config-Wert fälschlich als ungültig meldete. Läuft nur über `paths:`-Filter, wenn `.github/renovate.json5` selbst angefasst wird — eine schema-ungültige Config ließe Renovate sonst auf seine Defaults zurückfallen, und Defaults heißt hier, dass die Expo-Regeln nicht greifen.
