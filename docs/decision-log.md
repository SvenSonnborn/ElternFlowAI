...
7. **ccache ist Voraussetzung, nicht Optimierung.** `RCT_USE_PREBUILT_RNCORE=0` heißt React Native aus dem Quellcode — kalt ~45–50 Minuten. Erst `USE_CCACHE=1` (das Expo-Podfile wertet die Variable in `ccache_enabled?` selbst aus, es ist kein Fremdkörper) plus persistenter Cache drückt das in den ~15-Minuten-Bereich, in dem ein blockierendes Gate zumutbar bleibt. `CCACHE_MAXSIZE` steht auf 1 G, weil der Cache-Key SHA-suffigiert ist: bei 2 G hätten wenige Pushes das 10-GB-Budget des Repos gefüllt und per LRU die Gradle- und CocoaPods-Caches verdrängt — der Cache hätte sich selbst aufgefressen.
