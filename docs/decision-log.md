...
- **Der Inline-Fehlerblock in beiden Event-Screens ist seit diesem Feature tot** (`{createMutation.error ? … }` in `EventCreateScreen.tsx`, `{updateMutation.error ? … }` in `EventEditScreen.tsx`): Beide Bedingungen können nicht mehr rendern, weil beide Screens wegnavigieren, bevor die Mutation settlet. Als Follow-up in `docs/TODO.md` festgehalten statt nebenbei entfernt — ihr Entfernen betrifft beide Screens und war in keiner der beiden Umsetzungs-Tasks enthalten.
