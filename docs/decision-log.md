...
4. **Der Route-Parameter `occ` trägt den Schlüssel.** Ein Deep-Link von vor diesem PR trägt noch ein aufgelöstes Datum; `useEvent`s `expanded.find(...) ?? expanded[0]`-Fallback greift für eine unverschobene Occurrence weiterhin (beide Felder sind dort gleich), bei einer verschobenen zeigt er eine andere Occurrence derselben Serie statt der gemeinten — kein Datenverlust, aber ein falsches Ziel.
5. **Der Schreibpfad rechnet in der Zone des Termins, nicht des Geräts.** `anchoredChanges`, der gemeinsame Helfer `mergeDateAndTimeOfDay` (`timezone.ts`, versorgt sowohl `applyOptimisticChanges` als auch `anchoredChanges`) und `recurrenceToRrule` nehmen ihre Wanduhrkomponenten jetzt in `master.timezone`/`occurrence.timezone`/`vars.timezone` statt in der Geräte-Zone. Möglich wurde das erst, weil `consumedBefore` und der Startdatums-Vergleich in `applyEditScope`/`applyDeleteScope` seit Decision 2 ebenfalls über `zonedDateKey(d, master.timezone)` statt lokaler Getter laufen — vorher waren alle vier Stellen an denselben Geräte-Datumsraum gebunden, und ein isolierter Fix an nur einer hätte die Diskrepanz nur verschoben.
