...
5. **`flush` läuft nur auf `background`, nicht auf `inactive`.** Wechselt die App in den Hintergrund, werden alle offenen Löschungen sofort committet — sonst würde ein App-Kill im Fenster die Mutation verschlucken, das Item bliebe lokal weg und käme beim nächsten Refetch kommentarlos zurück. Der `AppState`-Listener reagiert dafür ausdrücklich nur auf `background`, nicht auf `inactive`: Auf iOS tritt `inactive` auch beim Herunterziehen des Kontrollzentrums, bei einer eingehenden Anruf-Einblendung und in der App-Switcher-Vorschau auf. Dort zu committen nähme dem Nutzer das Undo-Fenster weg, ohne dass er die App tatsächlich verlassen hätte.
