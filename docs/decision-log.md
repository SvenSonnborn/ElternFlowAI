...
5. **Der Schreibpfad rechnet in der Zone des Termins, nicht des Geräts — für `anchoredChanges`, `mergeDateAndTimeOfDay` und `recurrenceToRrule`.** Diese drei nehmen ihre Wanduhrkomponenten jetzt in `master.timezone`/`occurrence.timezone`/`vars.timezone` statt in der Geräte-Zone (`mergeDateAndTimeOfDay` in `timezone.ts` versorgt sowohl `applyOptimisticChanges` als auch `anchoredChanges`). Möglich wurde das erst, weil `consumedBefore` und der Startdatums-Vergleich in `applyEditScope`/`applyDeleteScope` seit Decision 2 ebenfalls über `zonedDateKey(d, master.timezone)` statt lokaler Getter laufen — vorher waren alle vier Stellen (diese drei plus der Datumsvergleich aus Decision 2) an denselben Geräte-Datumsraum gebunden, und ein isolierter Fix an nur einer hätte die Diskrepanz nur verschoben. Die Aussage gilt für genau diese Stellen, nicht für „den Schreibpfad" insgesamt: `toAllDayRange` ([dateRange.ts](../features/calendar/dateRange.ts)), das `EventEditScreen` beim Speichern eines Ganztages-Termins aufruft, rechnet weiterhin mit lokalen Gettern (`startOfDay`, `set({ hours: 23, minutes: 59 })`) in der Geräte-Zone — vorbestehend, nicht Teil dieses PRs, offene Ausnahme in [docs/TODO.md](./TODO.md).
6. **`canApplyOptimistically`s Datumsvergleich bleibt bewusst unangetastet.** Der client-seitige `format(new Date(args.changes.start_at), "yyyy-MM-dd") === args.occurrenceKey`-Vergleich liest weiterhin in der Geräte-Zone — vor Decision 5 war das folgenlos, weil `anchoredChanges` denselben Fehler machte und beide Seiten deshalb übereinstimmten. Seit der Schreibpfad zonenkorrekt rechnet, kann dieses Overlay einmal zu viel oder zu wenig zeigen, bis der ohnehin folgende Refetch korrigiert — kein falscher gespeicherter Wert. Außerhalb des Scopes dieser Iteration, siehe [docs/TODO.md](./TODO.md).

### Consequences

- Die drei Schreibpfad-Tests waren vor dem Fix unter `TZ=Europe/Berlin` grün und unter `TZ=America/New_York` rot — genau diese Asymmetrie war der Fehler, nicht ein einzelner falscher Wert. Sämtliche Suiten laufen unter drei Runner-Zonen (`Europe/Berlin`, `UTC`, `America/New_York`) mit identischem Ergebnis, 760 Tests.
...
- Zwei Grenzen bleiben offen und stehen in [docs/TODO.md](./TODO.md): Deep-Links von vor diesem PR landen auf der falschen Occurrence derselben Serie (Decision 4), und `canApplyOptimistically`s Geräte-zonen-Vergleich ist nicht mehr durch Symmetrie gedeckt (Decision 6).
