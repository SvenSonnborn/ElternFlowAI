...
1. **Gehostete Mend-App statt self-hosted Action.** Das Repo ist public, die App damit kostenlos: kein PAT als Secret, keine Actions-Minuten, kein Renovate-Image zu pflegen. Der einzige relevante Nachteil — kein Zugriff auf private Registries — trifft dieses Projekt nicht. Die Konsequenz prägt Entscheidung 2: `postUpgradeTasks` wird von `allowedCommands` gegated, und das ist eine **self-hosted-only Admin-Option**. Renovate kann in dieser Variante nicht selbst `expo install --fix` ausführen.
...
5. **Ein `customManagers`-Eintrag koppelt `bun-version:` in `ci.yml` an `mise.toml`.** Renovates `github-actions`-Manager fasst nur `uses:`-Zeilen an, keine `with:`-Inputs — ohne diesen Manager driftet `local == CI` beim ersten Bun-Update auseinander. Beide Fundstellen laufen über `matchDepNames: ["bun"]` (Regel 8) in einen gemeinsamen PR.
...
- Der Config-Validator-Aufruf (`renovate-validate.yml`) pinnt explizit auf `renovate@latest` statt ein bare `--package renovate`: Letzteres lässt npx jeden zwischengespeicherten Cache-Treffer wiederverwenden — während der Umsetzung zog das lokal einen `renovate@37.440.7` von Juli 2024 und meldete einen korrekten Config-Wert fälschlich als ungültig. Ohne die Pinnung könnten lokale Läufe und CI unbemerkt gegen unterschiedliche Renovate-Versionen validieren.
