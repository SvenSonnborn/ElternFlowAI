...
- **Die Datei heißt lokal `20260827080254_…`**, weil `apply_migration` den Versionsstempel selbst vergibt; der lokale Name folgt dem entfernten, damit ein späteres `supabase db push` sie nicht für unangewendet hält. (`20260814120000_seed_recipes.sql` trug dieselbe Abweichung aus einem früheren Lauf; sie wurde am 2026-09-01 nachgezogen — die Datei heißt seitdem `20260814084006_seed_recipes.sql`.)
...
5. **`flush` läuft nur auf `background`, nicht auf `inactive`.** Wechselt die App in den Hintergrund, werden alle offenen Löschungen sofort committet. Das **verengt** den App-Kill-Fall, beseitigt ihn aber nicht: `flush()` stößt die Requests nur an und hält die App nicht am Leben — iOS suspendiert kurz nach `applicationDidEnterBackground`, und `commit()` wartet auf ein nacktes `fetch` ohne Background-Task. Ohne den Listener bliebe das Fenster dagegen vollständig offen: Ein App-Kill verschluckte die Mutation, das Item wäre lokal weg und käme beim nächsten Refetch kommentarlos zurück. Der `AppState`-Listener reagiert dafür ausdrücklich nur auf `background`, nicht auf `inactive`: Auf iOS tritt `inactive` auch beim Herunterziehen des Kontrollzentrums, bei einer eingehenden Anruf-Einblendung und in der App-Switcher-Vorschau auf. Dort zu committen nähme dem Nutzer das Undo-Fenster weg, ohne dass er die App tatsächlich verlassen hätte. **Auf Web hängt der Listener gar nicht erst ein.** `react-native-web` kennt nur `active` und `background` und bildet `visibilitychange → hidden` direkt auf `background` ab — Tab-Wechsel, Fenster minimieren und Bildschirmsperre sind dort also genau das, wovon `inactive` auf iOS ausgenommen wurde. Ein Tab-Wechsel für zwei Sekunden hätte das Undo-Fenster geschlossen und die Löschung committet; die Begründung dieses Absatzes trägt auf Web damit nicht, und `useFlushPendingDeletes` kehrt dort um, bevor es registriert.
...
## ADR-027 — Optimistische Kalender-Updates: ein Occurrence-Overlay auf der Anzeige, keine Cache-Patches (2026-08-31)

### Status

Accepted. Baut auf dem Undo-Fenster aus [ADR-026](#adr-026--rückgängig-nach-dem-löschen-verzögern-statt-wiederherstellen-2026-08-31) auf: Löschen zeigt sich dort bereits sofort. Hier kommen Anlegen und Bearbeiten dazu. Keine Supersession.

### Context

Alle drei Kalender-Mutationen liefen bislang auf invalidate-and-refetch: `useCreateEvent`, `useUpdateEvent` und `useDeleteEvent` werfen ihr `invalidateQueries` in `onSuccess` und warten auf den Refetch. `docs/TODO.md` führte das seit der Kalender-V1 (ADR-008) als offenen Punkt.

Zwei Annahmen des Auftrags hielten der Realität nicht stand. **Löschen ist bereits sofort** — seit ADR-026 blendet `useFamilyEvents` einen Termin im Undo-Fenster aus, das Ziel ist dort auf anderem Weg erreicht. Und ein `onMutate`, das den Cache patcht, trifft nicht, was die UI zeigt: Der Query-Cache hält **Master-Zeilen**, die Oberfläche zeigt **Occurrences**, und dazwischen liegt `expandEvents`.

Im Haus gibt es dazu eine Gegen-Präzedenz: `useCreateTask`s Docstring lehnt Optimismus für das Anlegen ausdrücklich ab, mit drei Gründen — erfundene Id, fehlende gejointe Typ-Zeile, ein Rollback, der dem Nutzer das gerade Angelegte wieder wegnimmt. Zwei dieser Gründe entfallen beim Kalender (Decision 4).

Umgesetzt wurde ein kalender-eigener Store (`features/calendar/optimisticEvents.ts`, `useOptimisticEventsStore`) mit zwei Eintragsarten — `create` (eine synthetische Master-Zeile) und `update` (Event-Id, Occurrence-Datum, Scope, `EventChanges`) —, zwei reinen Anwendungsfunktionen (`applyOptimisticChanges`, `withOptimistic`) und einer Komposition (`visibleOccurrences`), die das Overlay mit dem bestehenden Pending-Delete-Filter aus ADR-026 verkettet. Verdrahtet ist das in `useCreateEvent` (`features/calendar/createMutation.ts`) und `useUpdateEvent` (`features/calendar/mutations.ts`); `EventCreateScreen.tsx` und `EventEditScreen.tsx` schließen jetzt sofort und melden einen Fehlschlag per Toast.

### Decisions

1. **Anzeige modellieren, nicht Speicherung.** Das Overlay liegt auf dem expandierten Occurrence-Strom, nicht auf den gecachten Master-Zeilen. Die Alternative — `onMutate` mit `setQueriesData` über alle `calendarKeys.range`-Einträge, wie `patchTaskCaches` es bei den Aufgaben tut — scheiterte an derselben Stelle wie beim Löschen: Um eine Änderung an _einer_ Occurrence sichtbar zu machen, müsste eine `event_exceptions`-Zeile synthetisiert werden, für `forward` die Serie gespalten — die Client-Nachbildung von `applyEditScope`, die ADR-026 Decision 2 für `applyDeleteScope` bereits verworfen hat. Dazu überholt jeder Refetch im offenen Fenster einen Snapshot-basierten Rollback.

2. **`useDeleteEvent` bleibt unangetastet.** Das Ziel ist dort bereits erfüllt: Ein gelöschter Termin verschwindet sofort, weil das Undo-Fenster aus ADR-026 ihn ausblendet. Ein `onMutate` nachzurüsten brächte keine sichtbare Verbesserung und holte die verworfene Client-Nachbildung zurück.

3. **Näherung statt Exaktheit bei Serien — aber nur, solange sie sichtbar zurechtrückt.** Die optimistische Anzeige muss gut aussehen, nicht dem entsprechen, was der Server schreibt; der Refetch korrigiert innerhalb einer Sekunde. Konkret nicht abgedeckt: eine per Exception verschobene Occurrence im `forward`-Fall — das Overlay vergleicht aufgelöste Daten, der Server rechnet auf Regel-Daten. Dieselbe Divergenz ist für den Lösch-Filter bereits in `docs/TODO.md` notiert; sie ist selbstheilend und kostet nur ein sichtbares Zurechtrücken.

   **Die Grenze dieser Näherung zog die Schluss-Review** (`all`/`forward` mit geändertem **Datumsanteil**). Der Server schreibt `changes.start_at` per `updateMaster` in `events.start_at` — und das ist `dtstart` der RRULE, die ganze Serie wandert also mit. Das Overlay kann nur die Tageszeit neu verankern und lässt jedes Occurrence-Datum stehen. Nachgerechnet an einer monatlichen Serie am 5., verschoben auf den 7.10.: Das Overlay zeigt `[09-05, 10-05, 11-05]`, der Refetch liefert `[10-07, 11-07, 12-07]`. Das ist kein Zurechtrücken mehr, sondern die Anzeige der **Nicht-Änderung genau der Eigenschaft, die der Nutzer geändert hat** — „Elternabend verschiebt sich auf den 7." → Datum ändern → „Alle Termine", und im Kalender steht alles unverändert. Der Nutzer hält das Speichern für fehlgeschlagen, eine Sekunde später springt die Serie. Deshalb bekommt dieser Fall **gar keinen** optimistischen Eintrag; entschieden wird das in `canApplyOptimistically` (`features/calendar/optimisticEvents.ts`), einer reinen Funktion neben `applyOptimisticChanges`, damit die Fallunterscheidung ohne React testbar ist. Die frühere Formulierung „jede Occurrence bekommt die neue Tageszeit und **behält ihr Datum**" (Spec §2.3) stimmte nur für eine reine Uhrzeit-Änderung und ist dort korrigiert.

4. **Anlegen ist hier optimistisch machbar, anders als bei Aufgaben.** Von den drei Gründen in `useCreateTask`s Docstring entfallen zwei: Die gejointe Typ-Zeile ist da — der Anlegen-Screen ruft `useEventTypes()` ohnehin, `onMutate` liest sie warm aus dem Query-Cache —, und der Rollback nimmt dem Nutzer nichts, weil der Fehler-Toast die Eingaben hält und erneut schickt (Decision 5). Es bleibt die erfundene Id. Sie kommt aus einer **Modul-Sequenz** (`optimistic-<n>`), nicht aus Formularwerten — eine naheliegende Alternative, eine deterministische Id aus Startzeit und Typ, kollidierte real: Der Anlegen-Screen setzt beide Felder beim Öffnen auf feste Defaults (09:00, Typ `family`), zwei schnell hintereinander angelegte Termine trugen dieselbe Id und kollidierten im React-Key (`${eventId}-${occurrenceDate}-${date}`) von `KalenderScreen` und `DashboardScreen` — eine der beiden Occurrences verschwand bis zum Refetch. Die Sequenz-Id lebt ohnehin nur Sekundenbruchteile; eine Kollision über einen App-Lauf hinweg gibt es nicht.

5. **Der Fehler-Toast kommt aus dem Screen, nicht aus dem Hook, und nicht aus einem Per-Call-Callback.** `features/*` darf nicht aus `app-sections/*` importieren, wo `useToast()` liegt. Und ein Per-Call-`onError` liefe nie: Das Sheet ist unmontiert, bevor die Mutation antwortet, und TanStack Query ruft Per-Call-Callbacks dann nicht mehr — festgehalten in `features/tasks/mutateAsyncSurvivesUnmount.test.ts`. Beide Screens rufen deshalb `mutateAsync(vars).catch(…)` in einer Closure, die den Unmount übersteht, und die Retry-Aktion des Toasts schickt dieselben `vars` erneut. Derselbe Bau wie `useUndoableDelete` aus ADR-026.

6. **Keine visuelle Sonderbehandlung der optimistischen Occurrence.** Kein Ausgrauen, kein Spinner, kein Platzhalter-Stil — sie soll echt aussehen, das ist der Zweck. Bleibt sie es nicht, sagt es der Toast. Als Folge verlieren `cal.create.saving` und `cal.edit.saving` ihre letzte Verwendung und entfallen mit ihren Keys: Beide Sheets schließen jetzt sofort, statt einen Ladezustand zu zeigen.
...
8. **Zwei Overlays, nicht eins.** Pending-Deletes (ADR-026) und optimistische Änderungen operieren auf demselben Occurrence-Strom und haben verwandte Form; sie zu vereinen wäre der sauberere Zug. Bewusst nicht jetzt: Der Pending-Delete-Store ist gerade durch eine vollständige Review-Runde gegangen, und Löschen stand in diesem Auftrag nicht zur Disposition. Als Follow-up in `docs/TODO.md` festgehalten, fällig beim dritten Overlay.

9. **Erst filtern, dann patchen.** Eine Löschung gewinnt gegen eine gleichzeitige Bearbeitung: `visibleOccurrences` (`features/calendar/optimisticEvents.ts`) wendet zuerst `withoutPendingDeletes`, dann `withOptimistic` an. Der Grund ist ein Datumsproblem, kein Stilproblem: Ein Update mit Scope `this`, das einen Termin auf ein anderes Datum verschiebt, schreibt `occurrenceDate` neu. Liefe der Patch **vor** dem Filter, verglicht der Löschfilter das **neue** Datum gegen das **alte**, das die offene Löschung trägt — der Vergleich schlägt fehl, und die gelöschte Occurrence taucht verschoben wieder auf. In der gewählten Reihenfolge trifft der Löschfilter die Occurrence dagegen noch unter ihrer ursprünglichen Identität. Der einzige denkbare Nachteil — die Occurrences eines optimistischen Creates könnten den Löschfilter umgehen — ist unerreichbar, weil ein optimistischer Create eine synthetische Id trägt, die keine Löschung adressiert; der Fall ist damit ohne Kosten. Er bleibt schmal (dieselbe Occurrence müsste zugleich eine offene Löschung und eine offene Bearbeitung tragen, Letzteres nur per Deep Link ins Bearbeiten-Sheet während des Undo-Fensters erreichbar), aber ein Test hält ihn fest.

   Die Reihenfolge stand während der Umsetzung zweimal falsch in der Spezifikation, bevor sie diesen Stand erreichte. Die erste Fassung schrieb die umgekehrte Reihenfolge vor (erst patchen, dann filtern) und begründete sie damit, andersherum bliebe ein gelöschter Termin sichtbar, weil der Patch ihn wieder einführe — eine unwahre Behauptung: `withOptimistic` bildet im Patch-Zweig nur ab (`.map`) und fügt nie hinzu. Die zweite Korrektur betraf dann die Reihenfolge selbst, aus dem oben beschriebenen Grund. Zwei falsche Fassungen für denselben Satz Code sind der Beleg, wie leicht sich diese Reihenfolge unbemerkt umdreht.

   Die Reihenfolge lebt deshalb in einer eigenen Funktion, nicht inline im Hook — das ist eine Frage der Testbarkeit, keine Stilfrage. Inline im Hook könnte ein Test die Reihenfolge nur nachbauen; ein nachgebauter Test bleibt grün, wenn jemand den Hook zurückdreht. Genau das ist während der Umsetzung zweimal passiert, bevor `visibleOccurrences` als eigenständig testbare Funktion entstand.

### Consequences

- **Beide Sheets schließen sofort statt einen Ladezustand zu zeigen.** `EventCreateScreen` und `EventEditScreen` rufen `router.back()`, bevor die Mutation überhaupt feuert — der Termin bzw. die Änderung steht dank `onMutate` schon im Kalender und im Dashboard, weil beide denselben `useFamilyEvents`-Hook lesen.
- **Ein Fehlschlag meldet sich per Toast mit Retry, statt die Eingaben im offenen Sheet zu halten.** Der Nutzer steht zu diesem Zeitpunkt längst auf einem anderen Screen; die Retry-Aktion schickt dieselben Formularwerte erneut, statt ein Sheet mit den alten Eingaben wieder zu öffnen.
- **Der Inline-Fehlerblock in beiden Event-Screens war seit diesem Feature tot** (`{createMutation.error ? … }` in `EventCreateScreen.tsx`, `{updateMutation.error ? … }` in `EventEditScreen.tsx`): Beide Bedingungen konnten nicht mehr rendern, weil beide Screens wegnavigieren, bevor die Mutation settlet. Zunächst als Follow-up in `docs/TODO.md` festgehalten statt nebenbei entfernt — das war falsch, und die Schluss-Review hat es zurückgenommen: Beide Blöcke sind gelöscht, der TODO-Eintrag mit ihnen (siehe den Punkt oben).
- **`description` überlebt einen `this`-Patch nicht.** `expandEvents` liest das Feld immer von der Master-Zeile ([expand.ts](../features/calendar/expand.ts)), auch bei einer `modified`-Exception — `applyOverride` kennt `description` nicht. Der Server schreibt eine per Occurrence geänderte Beschreibung zwar ins Override-JSON, die Anzeige übernimmt sie aber nie. `applyOptimisticChanges` bildet deshalb bei Scope `this` bewusst weiter die **alte** Beschreibung ab, obwohl `changes.description` die neue trägt — ein Patch, der die neue zeigte, erzeugte genau das Flackern, das dieses Feature abstellen soll (erst da, dann vom Refetch weggenommen). Das Overlay spiegelt, was `expandEvents` anzeigt, nicht, was der Server schreibt.
- **`all`/`forward` mit geändertem Datum bekommt gar keinen optimistischen Eintrag** (Decision 3). Das Overlay zeigte dort die Nicht-Änderung; die Enthaltsamkeit ist dieselbe wie bei `vars.recurrence` darunter.
- **Der Overlay-Store hat einen Watchdog und wird beim Abmelden geleert.** Die Spec begründete den fehlenden Watchdog damit, der Eintrag lebe „genau so lange wie die Mutation" — genau diese Annahme hatte ADR-026 für den Pending-Delete-Store bereits geprüft und verworfen: `features/supabase/client.ts` setzt dem `fetch` kein Timeout, hinter einem Captive Portal settelt eine Anfrage weder, noch lehnt sie ab, und hier wartet `onSettled` zusätzlich auf den Refetch. Ohne Obergrenze stünde ein nie gespeicherter Termin für den Rest der Sitzung im Kalender **und** im Dashboard, ohne Toast und ohne Log. Der Timer sitzt am Store (`add(payload, timeoutMs)`, `OPTIMISTIC_TIMEOUT_MS` = 30 s wie `COMMIT_TIMEOUT_MS`), nicht im Hook: Der Hang liegt in der `mutationFn`, ein `Promise.race` im Hook käme also gar nicht mehr zum Zug — und ein Timer am Eintrag deckt jeden künftigen Aufrufer mit ab. Die Mutation wird ausdrücklich **nicht** abgebrochen; danach bestimmen wieder die Serverdaten, was angezeigt wird. `useSignOut` (`features/auth/mutations.ts`) räumt den Store als **dritten** Modul-Store neben `qc.clear()` und `flush()` ab — sonst stünde der Termin des Vorgängers im Kalender des nächsten Familienmitglieds auf demselben Gerät, mit Titel, Ort und Kind-Zuordnung. Die Importrichtung stimmt (`features/auth` → `features/calendar`, nie umgekehrt); importiert wird direkt aus `features/calendar/optimisticEvents`, weil das Barrel über `./hooks` das nativewind-Runtime hereinzöge.
- **`onSettled` invalidiert nur bei Erfolg.** Bei einem Fehler ist serverseitig nichts passiert, der Refetch brächte dieselben Daten zurück — und er kostet mehr als nichts: TanStack ruft `onError` → `await onSettled` → **dann erst** lehnt `mutateAsync` ab. Der Rollback liefe also sofort, der Fehler-Toast wartete aber hinter einem Refetch, der bei toter Verbindung mit `retry: 1` und Backoff ins Leere läuft; im Funkloch sähe der Nutzer seinen Termin kommen und wieder gehen, ohne ein Wort dazu.
- **Ein optimistischer Termin ist nicht antippbar.** Beide Listen machen jede Occurrence zu einem `Pressable`, der `/event/[id]` mit `occ.eventId` öffnet — für einen optimistischen Create ist das `"optimistic-7"`, `fetchEventById` schickte das gegen eine `uuid`-Spalte und PostgREST antwortete `22P02 invalid input syntax for type uuid`. `KalenderScreen` und `DashboardScreen` prüfen deshalb `isOptimisticEventId` und tun für die ein, zwei Sekunden bis zum Refetch nichts. Die beiden Alternativen wurden verworfen: Eine echte UUID als synthetische Id führte in einen ebenso toten „nicht gefunden"-Screen und nähme der Id ihre Erkennbarkeit im Log (Decision 4); `mapEventError` im Detail-Screen allein verhinderte die unnötige Navigation nicht — er ist trotzdem gesetzt, weil eine rohe englische Datenbankmeldung in einer deutschen Oberfläche unabhängig davon nichts zu suchen hat.
...
Umgesetzt wurde eine idempotente Migration (`20260901083335_realtime_calendar.sql`), ein Modul `features/calendar/realtime.ts` mit reinen Funktionen, einer React-freien Kernfunktion und einem dünnen Hook, sowie ein Dev-Screen unter `app/debug/realtime.tsx`.
...
7. **Der Debug-Screen bleibt im Repo, und sein `__DEV__`-Guard sitzt im Screen.** #51 und #52 brauchen genau dieses Fenster in den Datenstrom; der Preis ist eine dauerhafte Route. Die naheliegende Absicherung — den `<Stack.Screen>`-Eintrag in `app/_layout.tsx` unter `__DEV__` stellen — wirkt **nicht**: Expo Router registriert Routen aus dem Dateisystem, die Layout-Deklaration setzt nur Optionen. `/debug/realtime` wäre im Release-Build weiterhin per Deep-Link und im Web-Bundle per URL erreichbar. Der Guard steht deshalb in `RealtimeDebugScreen` selbst (`if (!__DEV__) return <Redirect href="/" />`), als eigene äußere Komponente, damit die Hooks der inneren unbedingt laufen. Der `__DEV__`-Zweig im Settings-Sheet bleibt zusätzlich — er versteckt den Einstieg, der Guard schließt die Route.
...
- **Ein leerer oder ungültiger PAT ist ab jetzt schlechter als vorher.** Bis zu diesem Commit fiel der Server auf OAuth zurück und lief zumindest zeitweise; jetzt schlägt er hart fehl. Wer den Header hat, **muss** einen Token haben — es gibt keinen Zwischenzustand mehr. Der Gegenwert gilt **unter einer Bedingung**: Supabase-PATs lassen sich mit Ablaufdatum anlegen — Preset-Dauer, freies Datum bis zu einem Jahr, oder `never expire`. **Nur bei `never expire` verschwindet das wiederkehrende Symptom wirklich.** Mit Ablaufdatum kehrt es am Stichtag zurück, dann ohne den OAuth-Fallback, der es bisher abgefedert hat — also als harter Fehlschlag statt als stiller. Wer ein Ablaufdatum wählt, braucht einen Erneuerungs-Termin; das Dashboard zeigt seit dem Usage-Tracking zusätzlich, welche Tokens ungenutzt sind.
...
## ADR-030 — Live-Sync für den Kalender: Broadcast from Database, ein Familien-Topic, ein Mountpunkt (2026-09-02)

### Status

Accepted. Löst [ADR-028](#adr-028--realtime-für-den-kalender-ein-kanal-pro-familie-delete-bleibt-unzuordenbar-2026-09-01) in den Decisions 2, 3, 4 und 6 ab; 5, 7, 8 und 9 bleiben gültig. Zweite von drei Realtime-Iterationen (#50 → **#51** → #52).

### Context

[ADR-028](#adr-028--realtime-für-den-kalender-ein-kanal-pro-familie-delete-bleibt-unzuordenbar-2026-09-01) hatte das Fundament gelegt: `events` und `event_exceptions` lagen in der Publikation `supabase_realtime`, `features/calendar/realtime.ts` hielt einen `postgres_changes`-Kanal pro Familie, ein Dev-Screen zeigte den Strom. Angeschlossen war nichts — das war Issue #51.

Der Auftrag nannte vier Zutaten: `useFamilyEvents` um `supabase.channel(...).on(...)` erweitern, bei INSERT/UPDATE/DELETE invalidieren, beim Unmount aufräumen, Race-Conditions abfangen. Vier seiner Annahmen tragen so nicht. Die Subscription gehört **nicht** in `useFamilyEvents` — der Hook hat drei Aufrufer, zwei davon dauerhaft gemountet (Decision 5). Ein Race-Condition-Schutz gegen eigene Ereignisse wäre nicht nur unnötig, sondern schädlich (Decision 6). Ein DELETE ist über `postgres_changes` gar nicht zuzuordnen — der Grund, warum diese Iteration den Transport wechselt statt einen Hook zu erweitern (Decision 1). Und der tragfähige Teil der Arbeit liegt nicht im Client, sondern in der Datenbank.

Umgesetzt wurde eine idempotente Migration (`20260902065203_realtime_family_broadcast.sql`: Publikation zurücknehmen, `select`-Policy auf `realtime.messages`, eine Trigger-Funktion für beide Tabellen), die Sync-Schicht `features/realtime/` (`topic` · `normalize` · `subscribe` · `coalesce` · `reconnect` · `status` · `dispatch` · `useFamilyRealtime`), der Kalender-Mapper `features/calendar/realtimeKeys.ts`, `<SyncNotice />` auf Kalender und Dashboard und der Umbau des Debug-Screens. `features/calendar/realtime.ts` entfällt samt seinem Test; das Kalender-Barrel verliert den Re-Export und exportiert stattdessen `calendarInvalidationKeys`.

### Decisions

1. **Broadcast from Database statt Postgres Changes.** `postgres_changes` wendet RLS nicht auf DELETE an — Postgres kann eine bereits gelöschte Zeile nicht mehr gegen eine Policy prüfen. Das ist zweierlei: funktional keine gezielte Invalidierung (der Payload trägt nur die Row-Id, eine gelöschte Exception ist ihrem Event nicht zuzuordnen), datenschutzseitig Existenz und Zeitpunkt **fremder** Löschungen bei jedem angemeldeten Client — und dazu eine Lastverstärkung, weil jede Löschung irgendwo bei jedem Client einen Refetch auslöste. Ein `after`-Trigger sieht die alte Zeile dagegen noch. Was davon im Payload landet, ist **tabellenspezifisch** und gehört als Vertrag festgehalten: Bei `events` trägt `old_record` die eigene `id` und die `family_id`; bei `event_exceptions` trägt es die `event_id`, und die Familie löst der Trigger per Lookup über sie auf — sie landet nicht im Payload, sondern ausschließlich im Topic-Namen. Der Client liest entsprechend: `calendarInvalidationKeys` nimmt bei `events` die Row-Id, bei Exceptions die `event_id`. Die Autorisierung läuft über RLS auf `realtime.messages` gegen `public.current_family_id()`. Supabase führt Broadcast inzwischen selbst als die empfohlene Methode. Löst ADR-028 Decisions 3 und 4 ab: `CalendarChange.eventId: string | null` entfällt mit seinem Modul — die Null-Variante _war_ die DELETE-Lücke —, und die dort abgewogene Denormalisierung von `family_id` auf `event_exceptions` erübrigt sich, weil der Trigger die `family_id` per Lookup über `OLD.event_id`/`NEW.event_id` holt, ohne Schema-Änderung, Backfill oder Synchron-Trigger.

2. **Die Tabellen verlassen die Publikation `supabase_realtime`.** Ohne diesen Schritt bliebe das Leck offen, egal was der Client tut: Jeder angemeldete Nutzer könnte `postgres_changes` weiterhin selbst abonnieren. Der Verschluss gehört in die Datenbank, nicht in eine Konvention. Die Migration aus #50 bleibt als Historie stehen; diese kehrt sie um.

3. **Ein Topic pro Familie: `family:<familyId>`, nicht `calendar:<familyId>`.** Ein WebSocket-Abo statt eines pro Feature, eine RLS-Policy statt einer pro Feature — und Aufgaben oder Essen anzuschließen ist später ein Trigger plus eine Zeile in `MAPPERS` (`features/realtime/dispatch.ts`), kein zweites Abo. Preis: Der Client bekommt Ereignisse zugestellt, die ihn nichts angehen, und filtert sie im Mapper weg (`calendarInvalidationKeys` gibt für eine fremde Tabelle `[]` zurück) — bei einer Familie eine vernachlässigbare Menge. Löst ADR-028 Decision 2 ab.

4. **Die Trigger-Funktion verzweigt erst nach Tabelle, dann nach `TG_OP` — nicht umgekehrt.** Die naheliegende Fassung, die auch die Supabase-Doku nahelegt, ist eine flache Verzweigung nach `TG_OP` mit `case when TG_TABLE_NAME = 'events' then NEW.id else NEW.event_id end` darin. Sie ist in PL/pgSQL nicht lauffähig: Feldzugriffe auf `NEW`/`OLD` werden für die **gesamte** Expression gegen den tatsächlich gebundenen Record-Typ aufgelöst, unabhängig davon, welcher `when`-Zweig zur Laufzeit gälte. `events` hat keine Spalte `event_id`, `event_exceptions` keine Spalte `family_id` — die Expression bricht deshalb bei **jeder** Mutation ab, egal welcher Zweig gemeint war. Genau das ist passiert: Die erste angewendete Fassung quittierte jedes Anlegen, Ändern und Löschen mit `ERROR 42703: record "new" has no field "event_id"`, und weil der Trigger `after` läuft, scheiterte damit die Mutation selbst, nicht nur ihr Broadcast. Die verschachtelte Fassung (`if TG_TABLE_NAME = 'events' then … else …`, darin je ein `if TG_OP = 'DELETE'`) stellt sicher, dass kein Ausdruck je ein Feld berührt, das der gebundene Zeilentyp nicht hat. Die Begründung steht als Kommentar im SQL, und §3.3 der Spezifikation wurde auf die gebaute Fassung korrigiert — ein bekannt fehlerhafter Codeschnipsel in einem Designdokument ist eine Falle für den nächsten Leser. Zwei weitere Eigenheiten derselben Funktion bleiben, wie die Spec sie hatte: kein `coalesce(NEW.x, OLD.x)` (auf DELETE ist `NEW` nicht zugewiesen) und ein früher Ausstieg bei `fid is null`, weil `on delete cascade` die Exceptions **nach** der Master-Zeile abräumt und der Lookup dann ins Leere liefe — das DELETE-Broadcast des Events deckt den Fall bereits ab.

5. **Ein Mountpunkt statt einer Subscription pro Hook.** `useFamilyRealtime()` läuft **einmal** in `ThemedStack` (`app/_layout.tsx`), neben `useFlushPendingDeletes()` und bewusst **vor** `<AuthGate>`: Der Gate rendert bei Redirects `<Redirect>` statt seiner Kinder, ein Abo darunter würde bei jedem Routenwechsel ab- und wieder aufgebaut. Im Hook selbst hieße es drei Kanäle auf einem Topic, weil `useFamilyEvents` drei Aufrufer hat (Kalender, Dashboard, Anlegen-Screen) und die ersten beiden dauerhaft gemountet sind. Der Issue-Titel wird damit nicht wörtlich erfüllt, sein Ziel — „die Kalender-Query invalidiert sich automatisch" — schon, und zwar für alle drei Aufrufer statt dreifach.

6. **Kein Sonderfall für die Overlays, keine Echo-Unterdrückung.** ADR-028 Decision 1 hatte die Sorge festgehalten, eine eingehende Invalidierung müsse sich mit den offenen Löschungen ([ADR-026](#adr-026--rückgängig-nach-dem-löschen-verzögern-statt-wiederherstellen-2026-08-31)) und dem optimistischen Overlay ([ADR-027](#adr-027--optimistische-kalender-updates-ein-occurrence-overlay-auf-der-anzeige-keine-cache-patches-2026-08-31)) „vertragen". Sie trifft ein Design, das ADR-027 ausdrücklich nicht gewählt hat: Beide Overlays sind Anzeige-Filter **hinter** der Query, nicht Cache-Patches. Eine offene Löschung steht serverseitig noch und kam auch bisher mit jedem Refetch zurück — `withoutPendingDeletes` filtert sie aus dem frischen Satz genauso heraus wie aus dem alten. Ein optimistischer Eintrag überlebt einen Refetch, weil `withOptimistic` **nach** der Expansion läuft und `onSettled` ihn weiterhin erst nach seiner eigenen Invalidierung freigibt. Eigene Echos werden nicht unterdrückt: Ein zusätzlicher Refetch nach der eigenen Mutation ist nie falsch, nur doppelt, und das Sammelfenster fängt den Großteil ab — eine Unterdrückung bräuchte einen Actor im Payload und verschluckte fremde Änderungen, die zufällig im selben Fenster liegen. Das gehört in diesen ADR und nicht nur in den Code, weil es der Grund ist, warum an der auffälligsten Stelle des Issues **nichts** steht.

7. **Invalidiert wird `["calendar","events"]` und `["calendar","event"]`, nicht `calendarKeys.all`.** Der `all`-Präfix ist `["calendar"]` und zöge `types` und `reminders` mit, die eine Termin-Änderung nie betrifft: `event_types` ist eine Nachschlagetabelle, `reminders` eine eigene Tabelle ohne Trigger. `calendarKeys` bekommt dafür die beiden Präfix-Konstanten `eventsRoot` und `oneRoot`. Das ist zugleich die praktische Hälfte der abgelösten DELETE-Lücke (ADR-028 Decision 4): Ein Lösch-Ereignis war keiner Event-Query zuzuordnen, es blieb nur die breite Invalidierung — mit `event_id` im Payload ist auch ein DELETE gezielt.

8. **300 ms Sammelfenster.** Eine Scope-Löschung fasst mehrere Zeilen an — Scope `this` schreibt eine Exception **und** rührt die Master-Zeile an, ein Scope-Wechsel schreibt mehrere Zeilen —; ohne Fenster wären das drei bis vier Refetches derselben Range-Query. Eingehende Änderungen laufen deshalb in eine Ref, ein `setTimeout` schließt das Fenster, dann bildet `dispatch` je Änderung die Schlüssel und `mergeInvalidationKeys` die Vereinigung. Die Verzögerung liegt unter der Wahrnehmungsschwelle für „sofort".

9. **`subscribeToFamilyChanges` ist `async` — und der Abbruch-Marker deckt mehr ab als das `await`.** `client.realtime.setAuth()` ist für private Kanäle Pflicht (ohne Zugriffstoken am Socket lehnt die Policy auf `realtime.messages` den Join ab) und asynchron; das korrigiert eine Notiz im entfallenen `features/calendar/realtime.ts`, ein `setAuth()` sei verzichtbar — für `postgres_changes` stimmte sie, für private Kanäle nicht. Der Effekt braucht damit ein `cancelled`: Läuft das `await` erst nach dem Unmount durch, wird der eben erzeugte Kanal sofort wieder entfernt, statt verwaist stehen zu bleiben. Denselben Marker prüfen aber auch die **Kanal-Callbacks**, und das ist der Teil, der erst in der Umsetzung auffiel: `removeChannel` ist asynchron und löst erst nach dem Leave-Ack auf — genau dort liefert das SDK `CLOSED` aus, also nach dem Cleanup, aus einer toten Closure heraus. Ohne den Guard schrieb sie noch `setStatus("closed")` und stellte einen Degraded-Timer, den niemand mehr besitzt: Nach einem Logout erschien der Offline-Hinweis für eine längst tote Verbindung, und bei einem Familienwechsel verfälschte die alte Closure zusätzlich `previousStatus` für den gerade gestarteten neuen Kanal.

10. **Ein Altkanal desselben Topics wird vor dem Neuanlegen entsorgt — und abgewartet.** `RealtimeClient.channel()` dedupliziert nach `realtime:${topic}`: Es durchsucht seine eigene Kanalliste und gibt bei einem Treffer denselben Kanal zurück. Ein Kanal verlässt diese Liste aber erst mit seinem eigenen Leave-Ack, nicht schon beim Aufruf von `removeChannel()`. Läuft der Effekt neu, während der vorige Kanal noch `leaving` ist — Fast Refresh, StrictMode, ein schneller Familienwechsel —, bekäme der neue Aufruf also den absterbenden Kanal zurück; `subscribe()` joint nur aus `closed` heraus und wäre darauf ein **stiller No-Op**, und das nachfolgende Leave-Ack würfe die eben gesetzten Bindings gleich wieder weg. Ohne diese Entsorgung wäre der Live-Sync nach jedem Fast Refresh tot — im Dev-Betrieb also praktisch immer, und dort fällt es am spätesten auf. Verglichen wird über das öffentliche, unpräfixte Feld `subTopic` statt über `RealtimeChannel.topic`: Dieser Code soll das SDK-interne `realtime:`-Präfix nicht kennen müssen.

11. **Der Debug-Screen öffnet keinen eigenen Kanal mehr, sondern liest einen Ringpuffer.** Der ursprüngliche Entwurf ließ ihn ein zweites Abo auf demselben Familien-Topic aufmachen. Zusammen mit Decision 10 ergibt das einen Defekt aus zwei einzeln richtigen Entscheidungen: Das zweite Abo räumte den einen App-Kanal aus `ThemedStack` ab, und wer `/debug/realtime` öffnete, riss den Live-Sync für den Rest der Sitzung ein — ausgerechnet mit dem Werkzeug, das ihn beobachten soll. `useFamilyRealtime` schreibt eingehende Broadcasts deshalb zusätzlich, **nur unter `__DEV__`**, in einen gedeckelten Ringpuffer im Status-Store (`recentChanges`, `DEBUG_CHANGE_LOG_LIMIT` = 50); der Screen liest Status und Liste aus derselben Quelle. Die Einträge tragen eine aus dem Puffer-Kopf abgeleitete Sequenznummer, weil `receivedAt` als React-Key nicht taugt — zwei Broadcasts teilen sich ohne Weiteres dieselbe Millisekunde. ADR-028 Decisions 7 und 8 bleiben unberührt: `__DEV__`-Redirect-Guard im Screen, hartkodierte Copy.

12. **Der Degraded-Zustand ist ein Timer im Hook, der ein Boolean in den Store schreibt — nicht ein Prädikat in der anzeigenden Komponente.** §7 der Spezifikation sah `isDegraded(status, elapsedMs)` als reine Funktion vor, ausgewertet von `<SyncNotice />`. Gebaut wurde `degradedDelayMs(status)` plus ein `setTimeout` in `useFamilyRealtime`, das `setDegraded(true)` ruft. Zwei Gründe, beide bindend: Ein Timer, der `setState` einer Komponente auslöst, fällt unter `react-hooks/set-state-in-effect` — in diesem Repo ein Error, derselbe Stolperstein wie in ADR-028 Decision 9 —, ein Store-Write nicht. Und ein Timer an einer Stelle schlägt einen pro anzeigendem Screen: `<SyncNotice />` steht auf Kalender **und** Dashboard, die gleichzeitig gemountet sind. Die Schwelle selbst bleibt bei zehn Sekunden, aus dem Grund, den die Spec nennt: Ein normaler Reconnect nach Bildschirmsperre dauert unter einer Sekunde, ohne die Schwelle blitzte bei jedem App-Wechsel ein Hinweis auf. `subscribed` räumt das Flag im Store mit ab; jeder andere Wechsel lässt es unberührt, weil über die Dauer der Timer entscheidet.

13. **Der Degraded-Timer wird nur bei einem tatsächlichen Statuswechsel neu gestellt.** Phoenix' interner Rejoin-Backoff pendelt sich nach den ersten Versuchen auf konstant zehn Sekunden ein — exakt `DEGRADED_AFTER_MS`. Bei jedem Status-Callback neu zu stellen verwechselte eine **Dauer** mit einem **Abstand**: Ein flatternder Kanal, der schneller als alle zehn Sekunden denselben Status erneut meldet, zeigte den Hinweis dann nie — also genau im Fall, für den er gebaut ist.

14. **Die Sync-Schicht steht über den Features, nicht neben ihnen.** `features/realtime/dispatch.ts` importiert `features/calendar`; `topic`, `normalize`, `subscribe`, `coalesce`, `reconnect` und `status` importieren kein Feature und bleiben damit unter `bun test` ladbar. Die Alternative — eine Registry, bei der jedes Feature sich beim Modul-Laden anmeldet — wäre ein impliziter Seiteneffekt beim Import und in Tests schwerer zu fassen als ein expliziter Aufruf. Der Barrel `features/realtime/index.ts` ist deshalb ausdrücklich **nicht** der Testeinstieg: Er exportiert `useFamilyRealtime` mit, das über `@/features/auth` → `AuthGate` → `ThemeProvider` die NativeWind-Runtime hereinzieht — dieselbe Einschränkung, unter der `features/auth` schon `features/calendar/optimisticEvents` direkt importiert.

15. **`sync.*` statt `cal.*` für die Offline-Copy.** Der Kanal ist familienweit; Aufgaben und Essen werden später dieselbe Zeile benutzen. Die beiden Keys stehen mit Deck-Eintrag in `docs/COPY.md` — die Datei gehört dem Designer und ist sonst gesperrt, die Bearbeitung war für diesen Eintrag ausdrücklich freigegeben (Konversation vom 2026-09-01). Ein Katalog-Key ohne Deck-Eintrag wäre genau die stille Divergenz, die die Regel verhindern soll.

16. **Kriterium für „Datei korrigieren" vs. „Folge-Migration": Würde ein frisches `db push` den Defekt reproduzieren?** Zwei Migrationsdateien wurden im Verlauf dieses ADRs scheinbar nach entgegengesetzten Prinzipien behandelt. Decision 4 hat die fehlerhafte Trigger-Funktion in der bereits angewendeten `20260902065203_realtime_family_broadcast.sql` korrigiert statt eine Folge-Migration anzuhängen: Ein frisches Setup, das diese Datei laufen lässt, hätte sonst denselben `42703`-Fehler reproduziert — Historien-Reinheit, die niemand konsumiert, wiegt weniger als eine Datenbank, die beim nächsten `db push` kaputt hochkommt. Der Kommentar in der **abgelösten** `20260901083335_realtime_calendar.sql`, der noch auf das inzwischen gelöschte `features/calendar/realtime.ts` verweist, blieb dagegen unverändert stehen: Diese Datei fügt ihre beiden Tabellen der Publikation nur hinzu, die nachfolgende Migration nimmt sie wieder heraus — ein frisches `db push` durchläuft beide fehlerfrei, die tote Referenz ist reine Prosa in bereits abgelöster Historie, die niemand als Quelle konsultiert. Beide Entscheidungen folgen damit demselben Kriterium, nicht zwei gegensätzlichen.

### Consequences

- **Jede Termin-Mutation schreibt jetzt zusätzlich eine Zeile in `realtime.messages`.** Etwas Schreiblatenz pro Mutation statt WAL-Mitlesen ohne Trigger — der Preis für gezielte, familiengebundene Ereignisse. Supabase partitioniert die Tabelle tagesweise und löscht Partitionen älter als drei Tage selbst; es gibt also nichts aufzuräumen. `replica identity full` bleibt aus und ist endgültig gegenstandslos, weil `OLD` aus dem Trigger kommt und nicht aus dem WAL (korrigiert ADR-028 Decision 6). Weil der Trigger `after` läuft, ist er außerdem nicht folgenlos für die Mutation selbst: Ein Fehler in **unserer** Trigger-Funktion lässt die schreibende Transaktion scheitern — siehe Decision 4.
- **Der Deck-Eintrag in `docs/COPY.md` steht auf ausdrücklicher Freigabe.** Die Datei gehört dem Designer und ist sonst gesperrt (CLAUDE.md, Non-Negotiable 1); für den `sync.*`-Abschnitt wurde die Bearbeitung am 2026-09-01 ausdrücklich erteilt, weil ein Katalog-Schlüssel ohne Deck-Eintrag genau die stille Divergenz wäre, die die Regel verhindern soll. Das steht hier und nicht nur in §7 der Spec, weil ein Diff-Leser die Freigabe im Decision-Log sucht und ihr Fehlen sonst zu Recht als Regelverstoß liest.
- **Ein verlorenes Broadcast dagegen ist stumm — kein Trigger-Fehler, sondern ein Zustellfehler.** `perform realtime.broadcast_changes(...)` in `public.broadcast_family_change()` ruft intern `realtime.send()`, Supabase-eigener Code außerhalb dieses Repos; der kapselt seinen `insert into realtime.messages` in `exception when others then raise warning` (bestätigt gegen den laufenden Katalog: `pg_proc.prosrc` von `realtime.send`). Scheitert dieser Insert — fehlende Tagespartition, Rechte, Quota —, gewinnt die Mutation trotzdem: Es geht kein Broadcast raus, kein Client wechselt seinen Status, und der Nachhol-Pfad greift nicht, weil `shouldRefetchAfterResubscribe` einen Statuswechsel voraussetzt, den es hier nie gibt. Der andere Client bleibt beliebig lange stumm veraltet, ohne `<SyncNotice />` — der einzige Ausfall, für den dieser Entwurf keine Antwort hat. Die Kehrseite ist der eigentliche Gewinn und mehr wert als jede Absicherung dagegen: Ein Realtime-Ausfall kann umgekehrt die Kalender-Schreibvorgänge nicht mitreißen, weil genau dieses `exception`-Handling in `realtime.send()` das abfängt.
- **Der Reconnect-Refetch holt den _Zustand_ nach, nicht die verpassten _Ereignisse_.** Broadcast Replay ist nicht eingeschaltet; nach jeder Unterbrechung ist der Cache stumm veraltet. Ein Statuswechsel zurück auf `subscribed` — aus `closed`, `timedOut` oder `error`, ausdrücklich **nicht** beim ersten Abonnieren — invalidiert deshalb einmalig die beiden Wurzeln, die ein Broadcast überhaupt hätte tragen können. Die Regel steckt in `shouldRefetchAfterResubscribe(prev, next)` und ist damit prüfbar; ein eigenes `AppState`-Handling braucht es nicht, weil der Socket beim Zurückholen der App ohnehin abbricht und neu aufbaut.
- **`useFamilyRealtime` selbst bleibt ungetestet** — dieselbe Lücke wie zuvor bei `useCalendarRealtime` (ADR-028), nur an einem neuen Modul: Es gibt im Repo keinen Pfad, auf dem React-Komponenten unter `bun test` rendern. Alles Prüfenswerte liegt darunter in reinen Funktionen, und genau dafür ist der Schnitt gemacht. In `docs/TODO.md` festgehalten, dort auf das neue Modul umgeschrieben.
- **Die Migration ist angewendet, und der Versions-Abgleich stimmt diesmal ohne Nacharbeit.** Anders als bei ADR-028 war der Supabase-MCP-Server in der Umsetzungssitzung authentifiziert; `list_migrations` führt `20260902065203_realtime_family_broadcast` — dieselbe Version wie der Dateiname, weil die Datei nach `apply_migration` gleich auf die vergebene Version benannt wurde (`supabase/SETUP.md` §3). Der Live-Betrieb ist damit auch der Grund, warum Decision 4 überhaupt einen Befund enthält statt einer Vermutung.
- **Nach einem abgelehnten `setAuth()` versucht es niemand erneut.** Der Fehlerzweig meldet `error` und loggt; der Kanal bleibt bis zu einem Wechsel von `familyId` tot, und der Nutzer sieht dauerhaft den Offline-Hinweis. Für den häufigen Fall — kurzer Netzausfall — trägt Phoenix' eigener Rejoin, aber der greift erst nach einem erfolgreichen Join. In `docs/TODO.md` festgehalten.
- **`subscribeToFamilyChanges` entsorgt Altkanäle ohne Zustandsprüfung.** Heute ist das richtig und es gibt genau einen Aufrufer; ein künftiger **zweiter** Abonnent desselben Topics würde den ersten aber stumm abräumen. Die Falle ist keine hypothetische — sie ist genau der Grund, warum der Debug-Screen umgebaut werden musste (Decision 11). Deshalb keine Verengung jetzt, aber ein Eintrag in `docs/TODO.md`, damit der nächste Aufrufer sie kennt, bevor er sie stellt.
- **Die `onSettled`-Blöcke der Kalender-Mutationen invalidieren weiterhin `calendarKeys.all`.** Der Realtime-Pfad benutzt die engeren Präfixe, die Mutationen ziehen `types` und `reminders` bei jedem eigenen Schreibvorgang weiter mit. Sie nachzuziehen wäre eine Zeile pro Block — aber eine, die drei Overlay-Interaktionen berührt (ADR-026, ADR-027) und deshalb ihren eigenen Testlauf verdient, nicht den einer Realtime-Iteration. In `docs/TODO.md`.
- **Der Kalender-Client bekommt Ereignisse, die ihn nichts angehen** — der Preis des familienweiten Topics. Solange nur `events` und `event_exceptions` einen Trigger haben, ist die Menge null; mit Aufgaben und Essen wächst sie, und der Filter bleibt eine Zeile im Mapper.
- **Conflict-Detection bleibt offen.** Zwei gleichzeitige Edits derselben Zeile gewinnt weiterhin der letzte Schreiber; das ist #52. Es gibt bewusst **keine** `insert`-Policy auf `realtime.messages` — Clients senden nicht auf diesen Kanal, alle Nachrichten kommen aus dem `security definer`-Trigger. Eine nachzureichen wäre der Schritt, den #52 prüft, falls Conflict-Detection Client-Broadcasts braucht.

## ADR-031 — Conflict-Detection: `updated_at` beleben, dreiwertig vergleichen, ein Dialog im Root-Layout (2026-09-04)

### Status

Accepted. Ergänzt [ADR-030](#adr-030--live-sync-für-den-kalender-broadcast-from-database-ein-familien-topic-ein-mountpunkt-2026-09-02); löst nichts ab. Dritte und letzte der drei Realtime-Iterationen (#50 → #51 → **#52**).

### Context

ADR-030 endet mit dem Satz, der dieses Issue eröffnet: „Conflict-Detection bleibt offen. Zwei gleichzeitige Edits derselben Zeile gewinnt weiterhin der letzte Schreiber; das ist #52."

Der Auftrag nannte vier Zutaten — Testsetup mit zwei Clients, Änderung in A und Prüfung in B, „`updated_at`-Vergleich vor Update", Toast/Alert bei Konflikt. Drei seiner Annahmen trugen nicht, und alle drei sind nachgemessen statt vermutet:

- **`events.updated_at` wurde nie hochgezählt.** Die Spalte existiert seit dem ersten Kalender-Schema ([20260529091933_calendar.sql](../supabase/migrations/20260529091933_calendar.sql)) mit `default now()`, aber kein Trigger fasste sie an — geprüft gegen `pg_trigger` der laufenden Datenbank: auf `events` hing allein `broadcast_events_changes` aus ADR-030. Der Vergleich hätte **nie** angeschlagen; das Feature wäre ein stiller No-Op gewesen, der Sicherheit behauptet, die er nicht liefert.
- **`event_exceptions` hatte die Spalte gar nicht** — nur `created_at`. Der Scope „Nur diesen Termin" (`modifyOccurrence`) schreibt aber genau dorthin, also ausgerechnet in den bei Serien häufigsten Pfad.
- **Ein Dialog im Screen wäre nie gezeichnet worden.** `EventEditScreen.onSave` ruft `goBackOrToKalender()` **vor** `save(vars)`, damit die optimistische Änderung sofort im Kalender steht ([ADR-027](#adr-027--optimistische-kalender-updates-ein-occurrence-overlay-auf-der-anzeige-keine-cache-patches-2026-08-31)). Das Sheet ist unmontiert, bevor der Server antwortet.

Gebaut wurde eine idempotente Migration (`20260904075041_conflict_detection.sql`: `public.set_updated_at()`, die neue Spalte auf `event_exceptions`, drei `before update`-Trigger), das Versions-Token [features/calendar/version.ts](../features/calendar/version.ts), die Feldvergleiche [features/calendar/conflict.ts](../features/calendar/conflict.ts) und [features/tasks/conflict.ts](../features/tasks/conflict.ts), zwei Fehlerklassen samt Mapper-Zweigen, ein Pre-Flight in `updateEvent`/`deleteEvent` plus Compare-and-Swap auf `updateMaster`, ein CAS in `useUpdateTask`, und der Dialog als Trio `conflictStore` · `ConflictDialog` · `ConflictDialogHost` im Root-Layout.

**Die Spezifikation beschreibt an einer Stelle nicht mehr, was gebaut wurde.** Sie sah einen **zweiwertigen** Feldvergleich vor: `theirs` gegen `mine`. Die Zwei-Client-Verifikation ([docs/superpowers/plans/2026-09-04-conflict-detection-verification.md](./superpowers/plans/2026-09-04-conflict-detection-verification.md), Schritt 6) hat gezeigt, dass der die **eigenen** Änderungen des Nutzers als Konflikte mitlistet: Das Formular schickt immer den vollen Feldsatz, ein geänderter Titel weicht also zwangsläufig von der Serverzeile ab, auch wenn ihn sonst niemand angefasst hat. Damit war die Feldliste nach **jedem** Versionssprung nicht-leer, die Regel „leere Liste → durchspeichern" feuerte nie, und der Dialog erschien bei jeder fremden Schreiboperation — genau das „wird weggeklickt", vor dem die Spec selbst warnt. Reproduziert identisch bei Termin (`Titel`+`Ort`) und Aufgabe (`Titel`+`Fach`); kein Zufall, sondern dieselbe Grundannahme in beiden Vergleichsfunktionen.

Der Umbau auf einen **dreiwertigen** Vergleich wurde deshalb dem Nutzer vorgelegt und von ihm freigegeben — er geht über den ursprünglich freigegebenen Plan hinaus. Wo Spec und Code auseinandergehen, gilt der Code; §7 der Spezifikation wurde auf die korrigierte Erwartung nachgezogen, aus demselben Grund wie in ADR-030 Decision 4: ein bekannt fehlerhafter Erwartungswert in einem Designdokument ist eine Falle für den nächsten Leser.

### Decisions

1. **Der `updated_at`-Vergleich braucht zuerst einen Trigger — die Migration ist die Vorbedingung, nicht das Beiwerk.** `public.set_updated_at()` bekommt bewusst **kein** `security definer`: Anders als `broadcast_family_change` (ADR-030) fasst die Funktion nur `NEW` an und schreibt in keine fremde Tabelle, erhöhte Rechte wären hier ausschließlich Angriffsfläche. `set search_path = ''` bleibt trotzdem gesetzt, weil es Hausstandard ist; `now()` liegt in `pg_catalog` und wird auch bei leerem Suchpfad gefunden. Die neue Spalte auf `event_exceptions` kommt **ohne Backfill**: Die Basis-Version stammt immer aus der Zeile, die der Client gerade gelesen hat — der erste Vergleich ist per Konstruktion konsistent, egal welcher Wert dort steht, und ab dem ersten fremden Schreibvorgang bewegt sich der Wert.

2. **`before update`, nicht `after`.** Ein `after`-Trigger sieht die Zeile, nachdem sie geschrieben wurde; sein `NEW` zu ändern hat keine Wirkung mehr. Die Reihenfolge zu ADR-030 fällt damit als Nebenprodukt richtig aus: Postgres führt alle `before row`-Trigger vor allen `after row`-Triggern aus, `set_updated_at` läuft also garantiert vor `broadcast_family_change`, und der Broadcast trägt das neue `updated_at` bereits in `record`. Das liest heute niemand — der Vertrag ist aber ohne weiteres Zutun konsistent, und wer ihn später liest, muss nichts umbauen.

3. **Das Versions-Token deckt genau die Zeilen ab, die geschrieben werden** — die Master-Zeile plus die Exception an diesem Datum, `${row.updated_at}|${exception?.updated_at ?? "-"}`. Beide Nachbar-Zuschnitte wären falsch: **Nur `events.updated_at`** übersieht jede fremde Änderung am Scope „Nur diesen Termin". **Die ganze Serie** (`max` über alle Exceptions) übersieht nichts, meldet aber einen Konflikt, wenn jemand eine _andere_ Occurrence derselben Serie ändert — und dieser Fehlalarm käme durch den Feldvergleich nicht heraus: Die fremde Änderung am 22. ändert nichts an der Auflösung des 15., wohl aber die eigene Eingabe. Bei Aufgaben ist die Version schlicht `task.updated_at`: eine Zeile, keine Ausnahmen, kein eigenes Modul.

4. **Pre-Flight ist der Detektor im Kalender, CAS die Absicherung — bei Aufgaben genau umgekehrt.** Die beiden Features sind verschieden gebaut, weil ihre Schreibpfade verschieden aussehen. `updateEvent`/`deleteEvent` lesen den Master **ohnehin** (`deps.fetchMaster`), weil `applyEditScope`/`applyDeleteScope` ihn für die RRULE-Arithmetik brauchen — der Vergleich ist dort kostenlos, deckt mit einer Stelle alle vier Scopes ab (auch den mehrstufigen Forward-Split, der `updateMaster` gar nicht ruft) und liefert die fremde Fassung für den Dialog gleich mit. `useUpdateTask` liest **nicht** vorher; ein Pre-Flight wäre dort ein Roundtrip pro Speichern, den niemand zahlt. Also trägt das ohnehin vorhandene `.select("id").maybeSingle()` ein `.eq("updated_at", vars.baseVersion)`, und gelesen wird **erst**, wenn null Zeilen zurückkommen — Zeile weg heißt der unveränderte `Error("Task no longer exists")`, Zeile da heißt `TaskConflictError(row)`. Im Normalfall kein zusätzlicher Roundtrip.

5. **`seenUpdatedAt` im CAS ist der gerade gelesene Stempel, nicht die `baseVersion` des Formulars.** Der Unterschied ist der ganze Zweck dieser zweiten Absicherung: Der Pre-Flight vergleicht gegen das, was das _Formular_ gesehen hat — ein Fenster von Minuten; das CAS auf `updateMaster` gegen das, was _dieser Schreibvorgang_ eine Zeile vorher gelesen hat — ein Fenster von Millisekunden. Mit `vars.baseVersion` prüfte das CAS dieselbe Bedingung zweimal und schlösse ausgerechnet das Fenster nicht, für das es da ist. `applyEditScope` hat `master` bereits in Scope und reicht den Wert an alle fünf `updateMaster`-Aufrufstellen durch. Der Exception-`upsert` und der Forward-Split bekommen **kein** CAS: Der `upsert` trägt kein `.eq`, und den `onConflict`-Vertrag der Unique-Constraint dafür aufzubrechen wäre ein Eingriff in korrekten, getesteten Code für Millisekunden — der Pre-Flight deckt beide Pfade.

6. **`baseVersion` und der Basis-Stand werden bei der Hydration eingefroren, nicht beim Speichern gelesen.** Das ist die tragende Invariante des Features: **`baseVersion` ist die Version, aus der der Formular-State entstand** — nicht die neueste, die die Query gerade führt. Der erste Bau las sie in beiden Screens zum Speicherzeitpunkt aus der lebenden Query (`occurrence.version` / `task.updated_at`), während das Formular nur einmal hydriert. Das macht den Guard in einem **alltäglichen** Ablauf blind, nicht in einem exotischen: ein `refetchOnMount` Millisekunden nach dem Öffnen (`staleTime: 30_000`), eine Invalidierung nach irgendeinem anderen Fehler, auf Web ein Fensterfokus — jedes davon schiebt die Version weiter, ohne dass Titel, Zeiten, Ort und Notizen mitziehen. Das CAS trifft dann anstandslos, überschreibt die fremde Änderung lautlos, und es erscheint **nie** ein Dialog. Neben `baseVersion` friert deshalb `baseOccurrence` bzw. `baseTask` im selben Moment ein; beide beschreiben denselben Stand und dürfen nicht auseinanderlaufen. Ausgenommen sind die Wiederholungsversuche aus dem Dialog — die berechnen bewusst die **frische** Version aus `theirs`/`err.row`, sonst kollidierten sie per Konstruktion erneut.

7. **Der Feldvergleich ist dreiwertig: ein Feld ist genau dann ein Konflikt, wenn `theirs ≠ base` und `mine ≠ theirs`.** Jemand anderes hat es geändert, **und** mein Schreibvorgang überschriebe es. `base` ist der Stand, aus dem das Formular hydriert wurde (Decision 6). Der zweiwertige Vergleich der Spezifikation (`theirs` gegen `mine`) listete die eigenen Änderungen mit, weil beide Formulare den vollen Feldsatz schicken und keinen Diff — `EventChanges` hat keine optionalen Felder, `toTaskChanges` liefert laut eigenem Docstring „the full editable field set, not a diff". Der Effekt war nicht kosmetisch: Die Liste war nach jedem Versionssprung nicht-leer, Decision 12 („leere Liste → durchspeichern") hatte damit **keinen erreichbaren Pfad**, und der Dialog erschien bei jeder fremden Schreiboperation mit offenem Formular. Aufgedeckt hat das erst der Zwei-Client-Lauf, für Termin und Aufgabe identisch; der Nachlauf belegt beide Richtungen: bei disjunkten Feldern genau **eine** Zeile (die fremde), bei beidseitig berührtem Feld weiterhin genau eine, bei inhaltlich leerem Versionssprung **kein** Dialog. Zwei Toleranzen bleiben aus der zweiwertigen Fassung erhalten, weil sie unabhängig davon nötig sind: `""` und `null` heißen beide „nicht gesetzt" (das Formular schickt `trim() || null`), und Zeitpunkte werden als Instant verglichen, nicht als Zeichenkette — PostgREST liefert `+00:00`, `toISOString()` liefert `Z`, ein Stringvergleich meldete jedes Speichern als Konflikt.

8. **Der Dialog wohnt im Root-Layout, nicht im Screen.** Das Edit-Sheet ist weg, bevor der Server antwortet — dieselbe Lehre, die [ADR-025](#adr-025--toast-ein-wirt-im-root-layout-ein-store-ohne-context-ort-nach-icon-abhängigkeit-2026-08-28) für den Toast gezogen hat, also dieselbe Bauform: ein Zustand-Store auf Modulebene ohne Context, eine stumpfe Darstellung, ein Wirt neben `<ToastProvider>`. Der `catch`-Block in `save()` ist eine Closure und überlebt den Unmount; er expandiert die fremde Fassung, baut die Vergleichszeilen und legt sie in den Store. Den Wirt zu vergessen heißt, dass Konflikte unbemerkt bleiben — nicht, dass etwas wirft.

9. **`Modal` statt `Alert.alert`.** `Alert.alert` ist unter react-native-web ein No-Op (`static alert() {}`); der Dialog wäre im Web-Bundle unsichtbar, und der Zwei-Browser-Test, den das Issue ausdrücklich verlangt, wäre nicht durchführbar gewesen. Gebaut ist deshalb `Modal transparent` plus zentrierte Card auf `theme.overlay` — dieselbe Bauform wie `DateTimePickerSheet.web.tsx`, damit auf iOS, Android **und** Web identisch, nur mit vorhandenen Tokens und ohne erfundenes Pattern-Doc. Dass genau dieser No-Op an anderer Stelle noch steht, hat der Testlauf teuer bezahlt (siehe Consequences).

10. **Zwei Wege, nicht drei.** „Deine Fassung speichern" schickt dieselben `vars` erneut, mit der frischen Version als Basis — kein `force`-Flag, kein Bypass, ein echter zweiter Versuch, der erneut kollidieren kann. „Andere Fassung behalten" schließt; der Realtime-Refetch hat die fremde Fassung längst im Kalender, die eigenen Eingaben sind verworfen. Ein drittes „Abbrechen" fällt mit dem zweiten zusammen, sobald kein Formular mehr steht — es gäbe nichts, wohin es zurückführen könnte. Echte drei Wege bekäme man nur, indem der Edit-Screen erst nach der Server-Antwort schließt; das nähme dem optimistischen Sofort-Schließen aus ADR-027 seinen Zweck.

11. **Im Aufgaben-Screen verlässt „Dialog zeigen" den Screen.** `TaskEditScreen` ist anders als das Kalender-Sheet noch montiert, wenn der Konflikt eintrifft — es hydriert aber nur einmal (`if (task && !hydrated)`). Bliebe der Screen stehen, trüge das Formular weiterhin die veralteten Eingaben, während `onSettled` die frische Version still nachgeladen hat: Der nächste Tap auf „Speichern" ginge mit frischer `baseVersion` **anstandslos** durch und überschriebe die fremde Änderung lautlos — derselbe blinde Pfad wie in Decision 6, nur einen Schritt später. `showConflict` ruft deshalb `goBackOrToTasks()`, ausschließlich im Konflikt-Zweig. Im Kalender stellt sich die Frage nicht, dort ist das Sheet ohnehin zu.

12. **Leerer Feldvergleich heißt: kein Dialog, der Schreibvorgang läuft mit frischer Basis durch.** Ein Versionssprung ohne inhaltliche Abweichung — jemand hat dasselbe geändert, oder etwas, das ich gar nicht anfasse — darf niemanden anhalten; ein Guard, der bei jedem Versionssprung meldet, wird weggeklickt und ist dann schlechter als keiner. Diese Regel stand von Anfang an in der Spec, war aber bis zum Drei-Wege-Umbau **praktisch tot** (Decision 7). Seit dem Umbau deckt sie einen echten, beobachteten Fall ab: Szenario 6b des Nachlaufs — B speichert unverändert (der `before update`-Trigger stempelt bei jedem `UPDATE`, unabhängig vom Inhalt), A ändert danach nur den Titel und speichert **durch**, ohne Dialog, und B's zwischenzeitliche Ortsänderung bleibt erhalten. Beim CAS-Fall ohne fremde Fassung (`EventConflictError(null)`) gibt es nichts zu vergleichen; dort erscheint der Dialog **ohne** Zeilen, weil die Alternative — stilles Durchwinken — genau der Fehler wäre, gegen den diese Iteration gebaut ist.

13. **Löschen meldet sich als Toast, nicht als Dialog.** Der Guard aus Decision 4 greift in `deleteEvent` unverändert; die Oberfläche nicht. Seit [ADR-026](#adr-026--rückgängig-nach-dem-löschen-verzögern-statt-wiederherstellen-2026-08-31) löscht die App verzögert — der Schreibvorgang läuft fünf Sekunden nach dem Tap aus `useFlushPendingDeletes`, der Nutzer ist längst auf einem anderen Screen und hat den Termin nicht mehr vor sich. Es gibt kein Formular, dessen Felder man nebeneinanderstellen könnte, und ein modaler Dialog aus dem Nichts wäre auf einem fremden Screen ein Übergriff. Also der Weg, für den `mapEventError` ausdrücklich gebaut wurde: `cal.error.conflict` im vorhandenen Fehler-Toast, mit der Aktion **„Trotzdem löschen"**, die dieselbe Löschung mit der frisch aus `err.row` berechneten Basis-Version wiederholt. Ein Bauteil, das schon steht, statt eines zweiten.

14. **`conflict.*` statt `cal.*`.** Ein Dialog für zwei Features — acht Schlüssel, fünf davon (`title`, `theirs`, `mine`, `keepMine`, `keepTheirs`) in beiden Screens; `body.event`/`body.task` sind je feature-eigen, und `deleteAnyway` gehört allein dem Lösch-Toast des Kalenders (Decision 13) —, also ein familienweiter Namespace, wie `sync.*` in ADR-030 Decision 15 eine Zeile für alle trägt. Feldnamen kommen bewusst **nicht** dazu: Der Dialog beschriftet seine Zeilen mit den vorhandenen `cal.edit.field*`- und `hw.*`-Schlüsseln, und die beiden Fehler-Keys bleiben feature-lokal (`cal.error.conflict`, `hw.error.conflict`), weil sie in feature-eigenen Toasts landen. „Deine Fassung", nicht „Ihre Fassung" — Letzteres liest sich im Deutschen als Höflichkeitsform und verstieße gegen die Du-Regel.

### Consequences

- **Der Deck-Eintrag in `docs/COPY.md` steht auf ausdrücklicher Freigabe.** Die Datei gehört dem Designer und ist sonst gesperrt (CLAUDE.md, Non-Negotiable 1); für den `conflict.*`-Abschnitt wurde die Bearbeitung am **2026-09-03** ausdrücklich erteilt — derselbe Vorgang wie bei `sync.*` in ADR-030 und aus demselben Grund: Ein Katalog-Schlüssel ohne Deck-Eintrag wäre genau die stille Divergenz, die die Regel verhindern soll. Das steht hier und nicht nur in §9 der Spec, weil ein Diff-Leser die Freigabe im Decision-Log sucht und ihr Fehlen sonst zu Recht als Regelverstoß liest. Der Diff ist eine reine Insertion (22 Zeilen, keine Löschung) an genau einer Stelle.
- **`updated_at` war auf _allen_ Tabellen tot; belebt wurden nur drei.** `families`, `parents` und `children` tragen dieselbe ungestempelte Spalte weiter. Sie bleiben draußen, weil kein Schreibpfad dieser Iteration sie berührt — fällig wird es, sobald eine dieser Tabellen ein Formular bekommt, das zwei Personen gleichzeitig offen haben können. In `docs/TODO.md` festgehalten, samt Kommentar am Ende der Migration.
- **Schritt 7 des Protokolls — der Lösch-Konflikt — ist auf Web nicht prüfbar.** `EventDetailScreen.onDeletePress` benutzt rohes `Alert.alert`, und `pickScope` fällt außerhalb von iOS ebenfalls darauf zurück; der Löschknopf tut auf Web schlicht **nichts** (zwei Versuche, pixelidentische Screenshots vor und nach dem Klick). Vorbestehend und seit Längerem in `docs/TODO.md` dokumentiert, durch diese Iteration aber teurer geworden: Der einzige Pfad, auf dem Decision 13 sichtbar wird, braucht jetzt einen Simulator- oder Gerätelauf. Die Ersatzprüfung auf einer Aufgabe war ausführbar, testet aber strukturell etwas anderes (siehe übernächster Punkt).
- **Aufgaben haben keine Realtime-Schicht.** ADR-030 deckt ausschließlich `events` und `event_exceptions` ab; `features/realtime/` kennt keinen Tasks-Pfad. Im Zwei-Client-Lauf musste B den Serverstand deshalb an mehreren Stellen per Reload nachladen statt per Live-Push. Erwartetes, dokumentiertes Scope-Verhalten und kein Fehler — die Konflikt-Erkennung selbst hängt nicht daran, wohl aber die Erfahrung danach: Wer „Andere Fassung behalten" wählt, sieht die fremde Fassung in beiden Listen unmittelbar, nur auf verschiedenen Wegen: im Kalender über den Broadcast, in der Aufgabenliste über `useUpdateTask.onSettled`, das `invalidateTasks(qc)` **auch im Fehlerfall** ruft und die Liste damit sofort nachladen lässt. Was den Aufgaben fehlt, ist der ungefragte Push: Ohne einen eigenen Schreibvorgang erfährt die Liste von einer fremden Änderung erst beim nächsten Refetch — im Lauf musste B deshalb per Reload nachladen.
- **`useUpdateTask` hat keine eigene Testsuite.** Der Hook spricht direkt mit Supabase; es gibt keinen injizierbaren Deps-Schnitt wie im Kalender (`fetchMaster` + `ops`), an dem sich das CAS ohne Netz prüfen ließe. Sein Compare-and-Swap ist damit allein über den Zwei-Client-Lauf belegt — beobachtet, nicht automatisiert. Ein Test bräuchte zuerst denselben Schnitt, den `updateEvent` schon hat. In `docs/TODO.md`.
- **Für die Hydrations-Invariante aus Decision 6 gibt es keinen Regressionstest**, obwohl sie das ganze Feature trägt: Sie lebt in zwei Screens, und im Repo gibt es keinen Pfad, auf dem React-Komponenten unter `bun test` rendern (dieselbe Lücke wie bei `useFamilyRealtime`, ADR-030). Belegt ist sie durch Codelektüre und den Zwei-Client-Lauf; gegen ein Zurückfallen auf `occurrence.version`/`task.updated_at` an der Speicherstelle schützt heute nichts als der Kommentar daneben. In `docs/TODO.md`.
- **Der Drei-Wege-Umbau hat ein zufälliges Sicherheitsnetz beseitigt.** `vars.recurrence` fließt nicht in den Feldvergleich ein — er kennt nur die fünf `EventChanges`-Felder. Solange zweiwertig verglichen wurde, war die Liste in einem gemischten Fall unfreiwillig nicht-leer: Ändert A die Regel **und** ein Feld, während B **nur** die Regel geändert hat, wich A's eigene Feldänderung immer von der Serverzeile ab, der Dialog erschien — aus dem falschen Grund, aber er erschien. Jetzt ist die Liste in genau diesem Fall leer, es wird lautlos durchgespeichert, und B's Regeländerung ist weg. Der Umbau ist trotzdem richtig (er repariert den häufigen Fall statt den seltenen zu bewachen), aber der Verlust gehört benannt; er steht als eigener Eintrag in `docs/TODO.md`.
- **Bei Scope „Nur diesen" zeigt die fremde Fassung immer die Master-Beschreibung.** `applyOverride` in [features/calendar/expand.ts](../features/calendar/expand.ts) liest `description` nicht aus dem Override zurück, obwohl `modifyOccurrence` es dorthin schreibt. Vorbestehend, durch den Feldvergleich aber erstmals tragend: Eine per Occurrence geänderte Notiz erscheint im Dialog mit einem Wert, den niemand gespeichert hat — und eine echte Abweichung in der Notiz bleibt unsichtbar. Stand bereits als eigener TODO-Eintrag im Bestand.
- **Eine per Override auf einen anderen Tag verschobene Occurrence fällt aus dem Versions-Schlüssel.** `occurrenceVersion` schlüsselt auf das **aufgelöste** Datum, `expand.ts` löst den Inhalt über das **regelerzeugte** auf; eine fremde Änderung an der inhaltsgebenden Exception wird dort übersehen. Keine neue Eigenheit — `modifyOccurrence` schreibt in diesem Fall ohnehin eine **zweite** Exception-Zeile am neuen Datum, statt die bestehende zu ändern, das Token folgt also dem, was der Schreibvorgang tut. Hier erstmals benannt statt eingeführt; in `docs/TODO.md`.
- **Beim Löschen einer _Aufgabe_ greift kein Guard.** `useDeleteTask` prüft keine `baseVersion`, und `TaskEditScreen.onDelete` übergibt `useUndoableDelete` kein `errorAction` — „Trotzdem löschen" existiert für Aufgaben im Code nicht. Im Lauf beobachtet: A's verzögerte Löschung gewinnt das Rennen bedingungslos und verwirft B's zwischenzeitliche Bearbeitung **lautlos**, ohne Toast, Rückfrage oder Hinweis. Der Auftrag nannte nur das Termin-Löschen; die Lücke ist damit außerhalb des Scopes, aber real und in `docs/TODO.md`.
...
## ADR-034 — Der Occurrence-Schlüssel: `occurrenceKey` trennt Regel-Datum von Anzeigedatum (2026-09-14)

### Status

Accepted. Ergänzt [ADR-008](#adr-008--kalender-v1-abgeschlossen-reminder-recurrence-editor-multi-day-2026-07-28), [ADR-032](#adr-032--alle-termine-verankert-die-serie-nicht-neu-2026-09-09) und [ADR-033](#adr-033--serien-werten-in-wandzeit-aus-eventstimezone-kein-tzid-2026-09-09). Löst nichts ab. Dritter von drei ADRs aus Block 1 der Roadmap (032 Anker → 033 Zonenmodell → 034 Occurrence-Schlüssel) — deckt D1 der beiden in [Roadmap 1.3](./roadmap.md#13-das-override-modell-verschwundene-exceptions--versions-schlüssel--erledigt) gebündelten Einträge ab. D2 (die Kandidatenmenge für aus dem Fenster verschobene Exceptions) bleibt offen.

### Context

`CalendarOccurrence` trug bislang ein einziges Datumsfeld, `occurrenceDate` — das von `expand.ts` aus dem aufgelösten Instant der Occurrence gebildete, mit lokalen Gettern in der Zone des Lesers formatierte Datum. Dasselbe Feld diente zugleich als Zeilenname in `event_exceptions` (`occurrence_date`), als Eingabe für den Versionsvergleich (`occurrenceVersion`), als React-Key in Kalender und Dashboard und als Routen-Parameter `occ` im Deep-Link. Solange eine Occurrence nicht per Override verschoben ist, ist das harmlos — Anzeigedatum und die Zeile, die sie meint, sind dasselbe Datum. Verschiebt `EventEditScreen` eine Occurrence mit Scope „Nur diesen" auf einen anderen Tag, fallen beide auseinander: Die Exception-Zeile liegt unter dem ursprünglichen, von der **Regel** erzeugten Datum, das Anzeigedatum zeigt den neuen Tag. Ein Schlüssel, der auf dem Anzeigedatum baut, trifft danach die falsche — oder gar keine — Zeile.

Drei stille Fehlschläge liefen aus genau dieser Verwechslung. `expand.ts` rief `occurrenceVersion(row, occurrenceDate)` mit dem aufgelösten statt dem Regel-Datum auf — ein vorbestehender Fehler, der beim Umbau auffiel und mitbehoben wurde: `occurrenceVersion` selbst war nie kaputt, der Aufrufer suchte nur die falsche Zeile, und eine fremde Änderung an der inhaltsgebenden Exception ging beim Konfliktvergleich deshalb unbemerkt durch. Ein zweites Bearbeiten derselben verschobenen Occurrence schrieb, weil dieselbe Verwechslung auch im Schreibpfad steckte, eine wirkungslose zweite Exception-Zeile statt die bestehende zu ändern, und meldete Erfolg. Ein Löschen mit „Nur diesen" ging aus demselben Grund ins Leere.

Die Zonenfrage dahinter wurde erst durch [ADR-033](#adr-033--serien-werten-in-wandzeit-aus-eventstimezone-kein-tzid-2026-09-09) sichtbar. Seit dort sind die Occurrence-**Instants** zonenkorrekt — `timezone.ts` rechnet an den Rändern zwischen Regel-Wandzeit und echtem Instant um —, die Datumsschlüssel selbst entstanden aber weiterhin mit lokalen `Date`-Gettern, also in der Zone des **Lesers**. Gemessen an einem täglichen Berliner Termin um 00:30 Ortszeit: derselbe Datensatz führt dieselbe Occurrence unter `TZ=Europe/Berlin` als `2026-06-03`, unter `TZ=America/New_York` als `2026-06-02`. Eine Absage vom Berliner Gerät aus griff auf einem Gerät in einer westlichen Zone am falschen Tag.

Eine zweite, unabhängige Folge derselben Verwechslung: Identität und Anzeigedatum sind nicht dasselbe. Ein Override kann eine Occurrence auf einen Tag verschieben, an dem die Serie **ohnehin** ein reguläres Vorkommen hat — beide trügen dann dasselbe `occurrenceDate`. Ein Gruppierungs- oder React-Key, der auf dem Anzeigedatum baut, kollidiert in diesem Fall (`spans.ts`s `groupSpans`, die Tageslisten in `KalenderScreen` und `DashboardScreen`); eine der beiden Occurrences verschwindet aus der Liste, ohne Fehler. Ein Regressionstest hält das fest und war vor der Umstellung nachweislich rot.

Der Schreibpfad hatte zusätzlich sein eigenes, symmetrisches Problem: `anchoredChanges` (`recurrence.ts`), `withTimeOfDay` (`optimisticEvents.ts`, inzwischen als gemeinsamer Helfer `mergeDateAndTimeOfDay` nach `timezone.ts` gezogen) und `recurrenceToRrule` (`createMutation.ts`, dort bestimmte `startAt.getDay()` das `rrule_byweekday`) nahmen weiterhin Wanduhrkomponenten in der Zone des **Lesers**. Ein Öffnen-und-Speichern mit Scope „alle" von einem Gerät in einer anderen Zone als `events.timezone` verschob damit die ganze Serie um eine Stunde. Diese drei Stellen ließen sich nicht isoliert auf `events.timezone` umstellen, solange `consumedBefore` und der Startdatums-Vergleich in `applyEditScope`/`applyDeleteScope` noch mit lokalen Gettern im Geräte-Datumsraum arbeiteten — alle vier Stellen mussten im selben Zonenraum rechnen, sonst hätte ein isolierter Fix die Diskrepanz nur verschoben statt behoben. Erst der zonenkorrekte `occurrenceKey` gibt beiden Seiten denselben Referenzraum: die Zone des Termins.

### Decisions

1. **`CalendarOccurrence` trägt zwei Datumsfelder statt eines.** `occurrenceKey` benennt die Zeile in `event_exceptions` und gehört dem **Termin**; `occurrenceDate` bleibt das aufgelöste Anzeigedatum und gehört dem **Leser**. Alles, was eine `event_exceptions`-Zeile meint — der Exception-Lookup in `expand.ts`, der Versionsvergleich (`occurrenceVersion`), `EventOps.cancelOccurrence`/`modifyOccurrence`, der Route-Parameter `occ` —, schlüsselt auf `occurrenceKey`. Ohne Override beziehen sich beide auf denselben Instant und sind gleich, solange Gerätezone und Terminzone denselben Kalendertag sehen — das ist der Normalfall, aber keine Garantie: Fällt die Tagesgrenze dazwischen, laufen beide trotz fehlenden Overrides auseinander (Decision 4).
2. **Der Schlüssel entsteht in `row.timezone`, das Anzeigedatum in der Zone des Lesers.** `expand.ts` bildet `occurrenceKey` über `zonedDateKey(occurrenceStart, row.timezone)`, `occurrenceDate` weiterhin über lokale Getter auf demselben Instant. `CalendarOccurrence` bekommt dafür ein eigenes `timezone`-Feld, das `row.timezone` mitträgt — der Schreibpfad (Decision 5) braucht ihn an der Oberfläche.
3. **Identität ist der Schlüssel, nicht das Anzeigedatum.** `spans.ts`s `groupSpans` und die React-Keys der Tageslisten in `KalenderScreen` und `DashboardScreen` bauen auf `occurrenceKey`, nicht auf `occurrenceDate` — weil ein Override eine Occurrence auf einen Tag verschieben kann, an dem die Serie ohnehin ein reguläres Vorkommen hat, und zwei Occurrences mit demselben Anzeigedatum sonst kollidieren.
4. **Der Route-Parameter `occ` trägt den Schlüssel.** Ein Deep-Link von vor diesem PR trägt noch ein aufgelöstes Datum — gebildet in der **Gerätezone** des Erstellers, nicht in `row.timezone`. Für eine unverschobene Occurrence, deren Ersteller im selben Kalendertag wie die Terminzone lag, greift `useEvent`s `expanded.find(...) ?? expanded[0]`-Fallback trotzdem (beide Felder waren zu dem Zeitpunkt gleich). Bei einer verschobenen Occurrence ist der Fall nicht so harmlos, wie „`find` findet nichts, fällt zurück" klingt: Weicht die Gerätezone des Link-Öffners von `events.timezone` so ab, dass die Tagesgrenze fällt (ein täglicher Berliner Termin um 00:30, geöffnet in `America/New_York`), trägt der alte Link z. B. `"2026-06-01"`, während der `occurrenceKey` derselben Occurrence `"2026-06-02"` lautet. `find` schlägt dann **nicht** fehl — es trifft die **Nachbar-Occurrence**, deren Schlüssel zufällig mit dem alten Datum übereinstimmt. Der Nutzer landet auf einem plausibel aussehenden, falschen Termin, und ein anschließendes „Nur diesen" schreibt oder löscht die Exception-Zeile des Nachbarn statt der gemeinten. Nur bei einem echten Nicht-Treffer greift `expanded[0]`. Der Vektor ist schmal (keine Push- oder Mail-Deep-Links, nur Browser-Tab oder Lesezeichen) — aber „kein Datenverlust, nur ein falsches Ziel" gilt seitdem nicht mehr uneingeschränkt.
5. **In der Zone des Termins statt der des Geräts rechnen `anchoredChanges`, `mergeDateAndTimeOfDay`, `recurrenceToRrule` und dessen Inverse `rruleToRecurrence` — und, seit der PR-121-Review auf diesem Zweig, zusätzlich `canApplyOptimistically` sowie die Fenstergrenzen-Berechnung für eine angeforderte Occurrence.** Die ersten drei nehmen ihre Wanduhrkomponenten in `master.timezone`/`occurrence.timezone`/`vars.timezone` statt in der Geräte-Zone (`mergeDateAndTimeOfDay` in `timezone.ts` versorgt sowohl `applyOptimisticChanges` als auch `anchoredChanges`). Möglich wurde das erst, weil `consumedBefore` und der Startdatums-Vergleich in `applyEditScope`/`applyDeleteScope` seit Decision 2 ebenfalls über `zonedDateKey(d, master.timezone)` statt lokaler Getter laufen — vorher waren alle vier Stellen (diese drei plus der Datumsvergleich aus Decision 2) an denselben Geräte-Datumsraum gebunden, und ein isolierter Fix an nur einer hätte die Diskrepanz nur verschoben. `rruleToRecurrence` — die Inverse von `recurrenceToRrule`, für den Rückweg vom gespeicherten `rrule_byweekday` zur Radio-Option im Bearbeiten-Formular — las den Wochentag zunächst noch mit `startAt.getDay()` in der Geräte-Zone, obwohl `recurrenceToRrule` beim Schreiben längst über `instantToFloating(startAt, timeZone)` in der Terminzone las: Für einen Start nahe Mitternacht widersprachen sich beide Richtungen, die Wochentags-Option kam `null` zurück, und `EventEditScreen` blendete den Wiederholungs-Editor für eine eigentlich darstellbare Regel aus. Noch auf diesem Zweig nachgezogen (Commit „`rruleToRecurrence` liest den Wochentag in der Terminzone"), aber bis zu dieser Überarbeitung in keinem ADR festgehalten. `anchoredChanges` hatte zusätzlich eine eigene, unabhängige Lücke: Der Anker saß von Anfang an korrekt in `master.timezone`, seine **Dauer** zwischen `changes.start_at` und `changes.end_at` wurde aber weiterhin absolut genommen — überquerte die bearbeitete Spanne eine Zeitumstellung, die verankerte aber nicht (oder umgekehrt), verschob eine absolute Addition die vom Nutzer gewählte End-Uhrzeit. Behoben nach demselben Muster wie `floatingDurationMs` in [expand.ts](../features/calendar/expand.ts) (ADR-033). Zwei weitere Stellen kamen mit der PR-121-Review dazu: `canApplyOptimistically`s Datumsvergleich (`optimisticEvents.ts`) — siehe Decision 6 für die Begründung, warum gerade diese Stelle erst nachträglich fällig wurde —, und die Fenstergrenzen-Berechnung für eine angeforderte Occurrence (`useEvent`, `EventEditScreen`s Konflikt-Dialog), die Tagesanfang/-ende eines angeforderten `occurrenceKey` bislang lokal statt in `row.timezone` bildete; jetzt `eventLookupWindow`, ein eigenes Modul `eventWindow.ts`, ausgelagert aus `hooks.ts`, weil `hooks.ts` über `design-system/ThemeProvider` transitiv `nativewind` importiert und deshalb unter `bun test` nicht ladbar ist. Die Aussage gilt für genau diese Stellen, nicht für „den Schreibpfad" insgesamt: `toAllDayRange` ([dateRange.ts](../features/calendar/dateRange.ts)), das `EventEditScreen` beim Speichern eines Ganztages-Termins aufruft, rechnet weiterhin mit lokalen Gettern (`startOfDay`, `set({ hours: 23, minutes: 59 })`) in der Geräte-Zone — vorbestehend, nicht Teil dieses PRs, offene Ausnahme in [docs/TODO.md](./TODO.md).
6. **`canApplyOptimistically` vergleicht seinen Datumswert jetzt in der Zone des Termins, nicht des Geräts.** Der Vergleich (`optimisticEvents.ts`) bildete `changes.start_at` bislang mit `format(new Date(args.changes.start_at), "yyyy-MM-dd")` in der Geräte-Zone, während `args.occurrenceKey` seit Decision 2 ein Regel-Datum in der Terminzone ist. Das stand hier ursprünglich als bewusst unangetastete Grenze — nicht, weil es unwichtig gewesen wäre, sondern weil es zu dem Zeitpunkt **folgenlos** war: `anchoredChanges` machte serverseitig denselben Geräte-Zonen-Fehler, und zwei falsche, aber übereinstimmende Werte ergeben ein richtiges Ergebnis. Genau diese Symmetrie ist mit Decision 5 gefallen — seit `anchoredChanges` (samt seiner Dauer) zonenkorrekt rechnet, konnte dieser Client-Check auf einem Gerät in einer anderen Zone als der des Termins einmal zu viel oder zu wenig zeigen, bis der ohnehin folgende Refetch korrigierte. Der Fehler war die ganze Zeit da; sichtbar wurde er erst, als die zweite Stelle verschwand, die ihn bis dahin gespiegelt hatte. Noch auf diesem Zweig behoben, vor dem Merge: Der Vergleich läuft jetzt über `zonedDateKey(new Date(args.changes.start_at), args.timezone) === args.occurrenceKey`, mit `timezone` als neuem Pflichtfeld an `canApplyOptimistically` und `UpdateEventVars` — der Aufrufer (`EventEditScreen`) hatte die Zone der Occurrence ohnehin schon zur Hand, dieselbe, die bereits an `recurrenceToRrule` geht. Die Folge blieb durchgehend auf die Anzeige beschränkt, nie auf den gespeicherten Wert — das galt vor wie nach dem Fix; der Docstring hält das jetzt als Einordnung fest, nicht mehr als offene Grenze.

### Consequences

- Die drei Schreibpfad-Tests waren vor dem Fix unter `TZ=Europe/Berlin` grün und unter `TZ=America/New_York` rot — genau diese Asymmetrie war der Fehler, nicht ein einzelner falscher Wert. Sämtliche Suiten laufen unter drei Runner-Zonen (`Europe/Berlin`, `UTC`, `America/New_York`) mit identischem Ergebnis — 760 Tests beim ursprünglichen Landen dieses ADRs, 766 nach Abschluss der PR-121-Review (neue Tests aus den Befunden B–D).
- Der vorbestehende Fehler in `expand.ts` (`occurrenceVersion` mit aufgelöstem statt Regel-Datum aufgerufen) ist mit behoben. Der zugehörige TODO-Eintrag „Eine auf einen anderen Tag verschobene Occurrence fällt aus dem Versions-Schlüssel" ist damit erledigt und entfernt.
- [ADR-033](#adr-033--serien-werten-in-wandzeit-aus-eventstimezone-kein-tzid-2026-09-09)s Consequences hatten die Schreibpfad-Lücke (Decision 5 hier) als neu benannt; beim Umbau kam zusätzlich der Forward-Schnitt als zweite, unabhängige Vermischung desselben Musters zutage (`occurrenceDate` entstand in der Geräte-Zone, `endOfDayInstant` interpretierte es in `master.timezone`). Beide sind mit diesem PR geschlossen; der zugehörige TODO-Eintrag ist entfernt.
- Eine Grenze bleibt offen und steht in [docs/TODO.md](./TODO.md): Deep-Links von vor diesem PR landen auf der falschen Occurrence derselben Serie (Decision 4). Die zweite, hier ursprünglich genannte Grenze — `canApplyOptimistically`s Geräte-zonen-Vergleich (Decision 6) — ist noch auf diesem Zweig geschlossen worden, bevor er gemergt wurde; der zugehörige TODO-Eintrag ist entfernt.
- [Roadmap 1.3](./roadmap.md#13-das-override-modell-verschwundene-exceptions--versions-schlüssel--erledigt) bleibt zur Hälfte offen: Der Versions-Schlüssel-Teil (D1) ist mit diesem ADR abgeschlossen, die Kandidatenmenge für aus dem Fenster verschobene `modified`-Exceptions (D2 — `expand.ts`s `expandRecurrence` müsste `event_exceptions` selbst einbeziehen) bleibt offen und bekommt einen eigenen PR.

## ADR-035 — Der Override-Vertrag: Kandidaten aus `event_exceptions`, `description` im Vertrag (2026-09-15)

### Status

Accepted. Ergänzt [ADR-008](#adr-008--kalender-v1-abgeschlossen-reminder-recurrence-editor-multi-day-2026-07-28), [ADR-033](#adr-033--serien-werten-in-wandzeit-aus-eventstimezone-kein-tzid-2026-09-09) und [ADR-034](#adr-034--der-occurrence-schlüssel-occurrencekey-trennt-regel-datum-von-anzeigedatum-2026-09-14). Löst nichts ab. Vierter ADR aus Block 1 der Roadmap: ADR-032/033/034 zählten drei, weil die Kandidatenmenge (D2) dort als offener Rest neben dem Occurrence-Schlüssel (D1) stand; dieser ADR schließt sie und damit [Roadmap 1.3](./roadmap.md#13-das-override-modell-verschwundene-exceptions--versions-schlüssel--erledigt).

### Context

ADR-034 hat eine per Override verschobene Occurrence **adressierbar** gemacht — `occurrenceKey` benennt seither die Zeile in `event_exceptions`, `occurrenceDate` das Anzeigedatum. Sichtbar war sie damit nicht. Die Kandidatenmenge einer Serie kam ausschließlich aus `rule.between(...)`, also aus Regel-Daten, und ein Override verschiebt eine Occurrence gerade von ihrem Regel-Datum weg.

Nachgemessen am 2026-09-15 an einer wöchentlichen Montagsserie ab `2026-06-01 09:00 Europe/Berlin`, deren Vorkommen vom 29.06. per Scope „Nur diesen" auf den 20.07. verschoben ist. Das Juli-Fenster (01.–31.07.) lieferte die Schlüssel

```
2026-07-06, 2026-07-13, 2026-07-20, 2026-07-27
```

— der Schlüssel `2026-06-29` fehlt, obwohl die Occurrence nach der Verschiebung mitten in diesem Monat liegt. Das Juni-Fenster (01.–30.06.) lieferte

```
2026-06-01, 2026-06-08, 2026-06-15, 2026-06-22
```

— dort fehlt sie ebenso, und zwar richtigerweise: Der Fensterfilter verwirft sie, weil sie im Juni nicht mehr stattfindet. **An beiden Daten unsichtbar**, ohne Fehler, ohne Meldung. Nach dem Fix führt dasselbe Juli-Fenster fünf Occurrences, die verschobene als erste — `occurrenceKey` `2026-06-29`, `occurrenceDate` `2026-07-20` —, das Juni-Fenster unverändert vier.

Der **Kollisionsfall** fällt damit mit an: Der 20.07. ist ein Montag, an dem die Serie ohnehin stattfindet. Nach dem Fix tragen zwei Einträge dasselbe `occurrenceDate` `2026-07-20` und die verschiedenen Schlüssel `2026-06-29` und `2026-07-20`. Dass beide überleben, trägt die Identitätsentscheidung aus [ADR-034](#adr-034--der-occurrence-schlüssel-occurrencekey-trennt-regel-datum-von-anzeigedatum-2026-09-14) — dort als vorausgedachter Fall begründet, hier zum ersten Mal tatsächlich eingetreten.

**`description` stand im Vertrag und wurde trotzdem nie gelesen.** Der Spalten-Comment von `event_exceptions.override` nennt seit der ersten Migration fünf anerkannte Keys — `title`, `description`, `start_at`, `end_at`, `location` —, und `modifyOccurrence` schreibt sie auch alle. `applyOverride` las vier davon. Gemessen an derselben Serie mit einer Exception am 08.06., die `title: "Geändert"` und `description: "Override-Notiz"` trägt: Der Titel kam an (`2026-06-08=Geändert`), die Beschreibung nicht — alle fünf Occurrences des Juni-Fensters zeigten `Master-Notiz`. `applyOptimisticChanges` verschluckte sie folgerichtig ebenfalls, mit einem Kommentar, der genau das begründete („Sie hier zu zeigen hieße, sie eine Sekunde später vom Refetch wegnehmen zu lassen").

**Ein kaputtes `override.start_at` leerte den gesamten Kalenderbereich.** `applyOverride` gab den Wert unbesehen in `new Date(...)`; ein `{"start_at":"kein-datum"}` ergab eine `Invalid Date`, und `format(resolved.startAt, "yyyy-MM-dd")` quittierte das mit `RangeError: Invalid time value`. Gemessen mit zwei Zeilen im selben Aufruf — der kaputten Serie und einem völlig unbeteiligten Einzeltermin: Der Wurf riss **beide** mit, weil `expandEvents` alle Zeilen des Fensters in einer Schleife abarbeitet. Über den App-Schreibpfad ist der Fall nicht erreichbar (`modifyOccurrence` schreibt ISO-Strings), über direkte DB-Schreibzugriffe, einen künftigen serverseitigen Writer oder eine Migration schon.

**Und eine weit entfernte verschobene Occurrence war über ihren eigenen Link nicht erreichbar.** `eventLookupWindow` baut sein Fenster um das angeforderte Regel-Datum; ein Override kann die Occurrence darüber hinausschieben. Gemessen an derselben Serie mit einer Exception am Regel-Datum `2027-10-18`, per Override auf den 25.10.2027 verschoben: Das Fenster endete bei `2027-10-18T21:59:59.999Z`, der Override-Start `2027-10-25T07:00:00.000Z` lag dahinter. `expandEvents` erzeugte die Occurrence, der Fensterfilter verwarf sie wieder, `find` lief leer, und der `expanded[0]`-Fallback zeigte **das erste Vorkommen der Serie** — einen anderen Termin, ohne jede Meldung. Dieselbe Klasse wie Befund B aus [PR #121](https://github.com/SvenSonnborn/ElternFlowAI/pull/121), der dieselbe Funktion für den unverschobenen Fall repariert hat. Mit der Weitung endet dasselbe Fenster bei `2027-10-25T08:00:00.000Z`.

### Decisions

1. **Die Kandidatenmenge hat zwei Quellen.** Regel-Vorkommen aus `occurrencesBetween` plus die Regel-Vorkommen jener `modified`-Exceptions, deren Override-Intervall das Fenster schneidet und deren `occurrence_date` nicht schon in der Regel-Menge liegt (`movedExceptionOccurrences` in [expand.ts](../features/calendar/expand.ts)). Dedupliziert wird auf `occurrenceKey`. Der `if (!occurrences.length) continue;`-Abbruch steht seither **nach** der Kandidatenberechnung: Eine Serie, die im Fenster kein Regel-Vorkommen hat, kann trotzdem eine hierher verschobene Occurrence haben.
2. **Ein Exception-Kandidat entsteht nur an einem echten Vorkommen der Regel.** Verwaiste Exceptions überleben den Löschpfad — `deleteAllExceptions` läuft nur bei `ruleDiffers`, `deleteExceptionsFromDate` nur ab dem Schnitt —, und ohne die Prüfung erzeugte eine solche Zeile einen Phantom-Termin an einem Datum, an dem die Serie nie stattfand. Geprüft wird mit einem gezielten `occurrencesBetween` über den Tag in der Zone des Termins: ein zusätzlicher rrule-Aufruf pro verschobener Exception, und die sind die Ausnahme.
3. **Zurückgegeben wird das Regel-Vorkommen, nicht der Override-Start.** Nur so läuft der Kandidat durch dieselbe Auflösung wie jedes andere Vorkommen und trägt hinterher denselben `occurrenceKey`, dasselbe Versions-Token und dieselbe Exception-Kennzeichnung — sonst wäre die Sichtbarkeit auf Kosten der in ADR-034 erkämpften Identität erkauft.
4. **`description` kommt in den Override-Vertrag, `all_day` nicht.** Ersteres ist reine Code-Arbeit — die Spalten-Doku nennt den Key seit dem ersten Tag. Zweiteres bräuchte `all_day` in `EventChanges` und dafür einen `cal.edit.*`-Copy-Key, also den Designer; es bleibt als 🎨-Eintrag in [docs/TODO.md](./TODO.md), dessen Code-Teil dieser ADR aber vorzeichnet.
5. **Der Vertrag bekommt ein eigenes Modul ([override.ts](../features/calendar/override.ts)).** Seit Decision 1 und 7 hat er zwei Leser — die Auflösung in `expand.ts` und die Fensterweitung in `eventWindow.ts` —, und zwei Antworten auf „ist das überhaupt ein Datum?" liefen beim nächsten Fix auseinander. `isJsonObject`, `overrideDate` und `overrideInterval` liegen dort zusammen.
6. **Ein unparsbarer Datumswert wird verworfen, nicht durchgereicht.** Dieselbe Antwort wie bei einer unbekannten Zone (Befund D, [ADR-033](#adr-033--serien-werten-in-wandzeit-aus-eventstimezone-kein-tzid-2026-09-09)): Ein Termin zur Regel-Zeit ist ungleich besser als ein leerer Kalender.
7. **Das Suchfenster einer angeforderten Occurrence deckt ihr Override-Intervall mit ab** — und **nur** ihres, nicht das jeder anderen Exception derselben Serie, sonst zöge eine einzige weit verschobene Exception das Fenster jedes Detail-Aufrufs mit und `expandEvents` expandierte Jahre statt Tage. Die Weitung wirkt in beide Richtungen, weil ein Override ebenso in die Vergangenheit wie in die Zukunft verschieben kann. `eventLookupWindow` nimmt dafür die ganze Zeile (`row`) statt dreier Positionsparameter — es braucht ihre Exceptions ohnehin, und `start_at` und `timezone` holt es sich seither selbst.
8. **Die Tagesgrenzen-Rechnung zieht als `zonedDayBounds` nach [timezone.ts](../features/calendar/timezone.ts).** Drittes Vorkommen — `eventLookupWindow` (beide Grenzen), `endOfDayInstant` in `recurrence.ts` (nur das Ende) und die Vorkommens-Prüfung aus Decision 2 (beide) —, damit fällig nach der Repo-Regel „wartet auf ein drittes Vorkommen". Sie liefert `null` statt zu werfen, weil die Aufrufer verschieden antworten müssen: `eventLookupWindow` fällt auf sein Standardfenster zurück, `endOfDayInstant` wirft — ein aus Müll abgeleitetes `until` kürzte eine Serie still am falschen Datum. **Präzisiert in derselben Iteration:** `zonedDayBounds` prüfte zunächst nur die **Form** des Schlüssels (`DATE_KEY_PATTERN`, `^\d{4}-\d{2}-\d{2}$`) — ein `Date.UTC`-Überlauf wie `"9999-99-99"` bestand dieses Muster anstandslos und `Date.UTC` normalisierte ihn still auf ein Datum in `+010007`. Nachgemessen an einer unbegrenzten Tagesserie: ein Suchfenster von 2.915.008 Tagen, `expandEvents` brauchte darüber 31,2 Sekunden für 2.912.292 Occurrences — und der Schlüssel ist über den Routen-Parameter `occ` direkt von einer URL aus erreichbar. Ein Rundlauf-Check der drei Komponenten über `setUTCFullYear` (nicht `Date.UTC`, das zweistellige Jahre auf 1900+ abbildet) fängt das jetzt vor der Grenzenbildung ab; `null` bedeutet seither auch „kalendarisch ungültig", nicht mehr nur „falsch geformt" — die Aufteilung zwischen den Aufrufern (Fallback vs. Wurf) bleibt dieselbe. **Zweiter Nachtrag, noch in derselben Iteration:** Die erste Fassung dieses Rundlauf-Checks validierte mit `setUTCFullYear`, baute die beiden Grenzen danach aber erneut mit `Date.UTC` — zwei Semantiken in einer Funktion, die für Jahre unter 100 auseinanderliefen: `zonedDayBounds("0099-06-15", …)` bestand die Prüfung (Jahr 99 ist gültig), das Ergebnis trug aber Jahr 1999, weil der zweite `Date.UTC`-Aufruf dieselbe Jahrhundert-Abbildung erneut auslöste, die der Check gerade ausgeschlossen hatte. Die Grenzen entstehen jetzt direkt aus dem bereits validierten `probe` (Tagesanfang) plus einer Kopie mit `setUTCHours(23, 59, 59, 999)` (Tagesende) — eine Konstruktion, ein `Date.UTC`-Aufruf weniger.
9. **Keine Datenmigration für Alt-Exceptions** (Spec §6.6). Zeilen, die unter dem alten Verhalten als zweite, wirkungslose Exception am aufgelösten Datum entstanden sind, bleiben **im Regelfall** wirkungslos: Ihr `occurrence_date` ist meist kein Vorkommen der Regel, Decision 2 hält sie draußen. Ausnahme ist der Kollisionsfall — trifft das aufgelöste Datum zufällig auf einen Tag, an dem die Serie ohnehin ein reguläres Vorkommen hat, findet die Schlüsselsuche in `expand.ts` die Zeile trotzdem — die Exception-Map wird ungefiltert aus **allen** Zeilen der Serie gebaut und bei jedem Regel-Vorkommen per `exceptions.get(occurrenceKey)` abgefragt — und wendet ihren Override auf dieses reguläre Vorkommen an. Vorbestehend, von diesem ADR nicht verschlimmert. Beide Fälle aufzuräumen bräuchte dieselbe serverseitige RRULE-Auswertung, die es nicht gibt — die Schlussfolgerung „keine Migration" bleibt deshalb auch für den Kollisionsfall richtig.

### Consequences

- **Vierzehn Tests konnten vor ihrem jeweiligen Fix nicht grün sein.** Zum Override-Vertrag neun: die fünf Fälle in `override.test.ts`, die mangels Modul nicht einmal luden (`Cannot find module './override'`), dazu „eine im Override geänderte Beschreibung gewinnt gegen die Master-Zeile", „ein explizites null im Override löscht die Beschreibung", „ein kaputtes Override-Datum leert nicht den ganzen Kalender" (`RangeError: Invalid time value`) und „patcht die Beschreibung — seit ADR-035 überlebt sie den Weg" in `optimisticEvents.test.ts`. Zur Kandidatenmenge drei: „ROT VOR DEM FIX: sie erscheint im Fenster ihres aufgelösten Datums", „ROT VOR DEM FIX: der Kollisionsfall — zwei Einträge am selben Anzeigetag" und „eine Serie, deren einziges sichtbares Vorkommen ein Kandidat ist, verschwindet nicht". Zur Fensterweitung zwei: die beiden „ROT VOR DEM FIX"-Fälle in `eventWindow.test.ts`. Sechs weitere Fälle tragen bewusst das Präfix **GRENZWÄCHTER** und waren schon vorher grün — sie sichern, dass der Fix die Occurrence nicht zusätzlich an ihrem alten Datum zurückbringt, dass ein Fenster über beide Daten sie genau **einmal** führt (die Deduplizierung), dass ein Override ohne `start_at` nichts weitet, dass der Override einer **anderen** Occurrence das Fenster nicht mitzieht, dass eine verwaiste Exception an einem Nicht-Vorkommen der Regel keinen Phantom-Termin erzeugt und dass eine `cancelled`-Exception keinen Kandidaten erzeugt — Letzteres auch dann nicht, wenn ihr Override selbst verschiebende Daten trägt.
- Sämtliche Suiten laufen unter drei Runner-Zonen mit identischem Ergebnis: `Europe/Berlin`, `UTC` und `America/New_York` liefern je **793 pass, 1 todo, 0 fail, 1585 expect() calls über 794 Tests in 69 Dateien**.
- Zwei Einträge aus [docs/TODO.md](./TODO.md) sind erledigt und entfernt: **„Aus dem Fenster verschobene `modified`-Exceptions verschwinden"** (Calendar-Sektion, ein CodeRabbit-Finding aus der Multi-Day-Iteration) und **„`applyOverride` kennt `description` nicht"** (Optimistic-UI-Sektion).
- Eine Grenze ist neu entstanden und steht in [docs/TODO.md](./TODO.md): **„Ein Override nur mit `start_at` wird strenger gefenstert als aufgelöst"**. Fehlt `end_at`, setzt `overrideInterval` das Ende gleich dem Start; die Vorprüfung der Kandidatenmenge verwirft damit ein Intervall, das der Fensterfilter dahinter behalten würde. Über den App-Schreibpfad nicht erreichbar, weil `modifyOccurrence` `EventChanges` vollständig schreibt. Vertagt, bis es einen zweiten Schreiber gibt; dann ist die richtige Form absehbar statt geraten. **Präzisiert in derselben Iteration:** `applyOverride` selbst ließ `endAt` zunächst beim unveränderten Regel-Ende der Occurrence stehen, wenn nur `start_at` gesetzt war — `endAt < startAt` war damit möglich (nachgemessen: `start=2026-07-20T07:00Z`, `end=2026-06-29T09:00Z`, drei Wochen davor). Ein CodeRabbit-Fix auf diesen PR lässt `endAt` seither der ursprünglichen Dauer der Occurrence folgen; das invertierte Paar ist damit weg, die reine Fenster-Asymmetrie zwischen `overrideInterval` (kennt die Basis-Dauer weiterhin nicht) und dem Fensterfilter (der die Dauer jetzt kennt) bleibt bestehen.
- **Die Sichtbarkeit endet am Fetch-Prädikat, nicht an der Kandidatenmenge.** Decision 1 behebt die Expansion, nicht das Laden: `fetchEventsInRange` ([features/calendar/queries.ts](../features/calendar/queries.ts)) lädt eine Zeile nur, wenn `start_at` vor dem Fensterende liegt **und** `rrule_until` entweder `null` ist oder nicht vor dem Fensterstart liegt. Verschiebt ein Override eine Occurrence per Scope „Nur diesen" über `rrule_until` hinaus oder vor `start_at` zurück, greift keine der beiden Bedingungen — die Master-Zeile wird für dieses Fenster gar nicht erst geladen, und die Kandidatenmenge sieht sie nie. Die Occurrence bleibt dann im Monatsraster unsichtbar, an beiden Daten, genau wie vor diesem ADR. Über einen Direktlink bleibt sie erreichbar, weil `useEvent` per `id` über `fetchEventById` lädt, nicht über das Range-Prädikat. Vorbestehend, von diesem ADR nicht behoben; die Grenze steht als eigener Eintrag in [docs/TODO.md](./TODO.md).
- **Ein Nebenbefund aus derselben Prüfung wurde mitbehoben, stammt aber nicht aus diesem ADR.** Der Rundlauf-Check von Decision 8 deckte auf, dass `zoneOffsetMs` ([timezone.ts](../features/calendar/timezone.ts)) dieselbe Jahrhundert-Abbildung enthält wie der ursprünglich in `zonedDayBounds` entfernte zweite `Date.UTC`-Aufruf — nur unabhängig davon und mit größerem Wirkradius: `asUtc` entstand dort per `Date.UTC(at("year"), …)`, und für jedes Instant, dessen Wandzeit-Jahr in der Zielzone unter 100 liegt, bildete das auf 1900+ ab. Gemessen: `zoneOffsetMs(new Date("0099-06-15T12:00:00.000Z"), "UTC")` lieferte `59958144000000` statt `0` — in **jeder** Zone, auch `UTC`, wo der Offset exakt `0` sein muss, weil `formatToParts` selbst korrekt `year: "99"` liefert (keine `era`, keine Sonderform; geprüft). Betroffen war damit potenziell jede Funktion, die über `instantToFloating`/`floatingToInstant` lokalisiert — `mergeDateAndTimeOfDay`, `zonedDateKey`, `zonedDayBounds` selbst —, nicht nur der hier behandelte Fall. **Dieser Bug stammt aus [ADR-033](#adr-033--serien-werten-in-wandzeit-aus-eventstimezone-kein-tzid-2026-09-09), dem allerersten Commit von `timezone.ts`, und wurde nicht von diesem PR eingeführt** — er wurde bislang nur nie ausgelöst, weil `zonedDayBounds`s eigener (jetzt entfernter) `Date.UTC`-Aufruf jedes Jahr unter 100 schon vor dem Aufruf von `floatingToInstant` auf 1900+ hochgemappt hatte und `zoneOffsetMs` deshalb nie ein rohes zweistelliges Jahr zu Gesicht bekam. Der Fix ändert ausschließlich die Konstruktion von `asUtc` (`new Date(0)` plus `setUTCFullYear`/`setUTCHours` statt `Date.UTC`); für Jahre ≥ 100 ist das Ergebnis nachweislich bit-identisch (u. a. die `America/Sitka`-1867-Fixture in `expand.test.ts` und alle DST-Tests bleiben unverändert grün).
- **`all_day` bleibt der einzige offene Punkt des Override-Vertrags** und ist 🎨-blockiert: Der Code-Weg ist mit `description` vorgezeichnet, der Blocker ist allein der fehlende `cal.edit.*`-Copy-Key für den Ganztages-Schalter im Bearbeiten-Formular.
- [Roadmap 1.3](./roadmap.md#13-das-override-modell-verschwundene-exceptions--versions-schlüssel--erledigt) ist damit vollständig — D1 mit [ADR-034](#adr-034--der-occurrence-schlüssel-occurrencekey-trennt-regel-datum-von-anzeigedatum-2026-09-14), D2 mit diesem ADR — und **Block 1 der Roadmap ist damit codeseitig abgearbeitet**: fünf PRs ([#117](https://github.com/SvenSonnborn/ElternFlowAI/pull/117) · [#118](https://github.com/SvenSonnborn/ElternFlowAI/pull/118) · [#119](https://github.com/SvenSonnborn/ElternFlowAI/pull/119) · [#121](https://github.com/SvenSonnborn/ElternFlowAI/pull/121) · dieser), vier ADRs, jeder mit einem Regressionstest, der vor dem Fix rot war. Offen bleibt allein der letzte Punkt der Definition of done dieses Blocks, die **Sichtprüfung der Serienbearbeitung am Simulator** — Web reicht dafür nicht, weil der Termin-Löschpfad dort gar nicht auslösbar ist (Block 3).
...
Accepted. Zwei verschiedene Verhältnisse zu [ADR-031](#adr-031--conflict-detection-updated_at-beleben-dreiwertig-vergleichen-ein-dialog-im-root-layout-2026-09-04), die nicht vermischt werden dürfen (dasselbe Muster wie [ADR-015](#adr-015--renovate-expos-versionshoheit-deckt-beide-vorgabe-kanäle-ab-2026-08-14) zu ADR-013):

- **Ergänzung.** Ergänzt außerdem [ADR-036](#adr-036--der-aufgaben-schreibpfad-bekommt-den-schnitt-des-kalenders-2026-09-17); löst keins von beiden ab.
- **Supersession** der letzten Aussage in ADR-031 Decision 12: „Beim CAS-Fall ohne fremde Fassung (`EventConflictError(null)`) gibt es nichts zu vergleichen; dort erscheint der Dialog **ohne** Zeilen, weil die Alternative — stilles Durchwinken — genau der Fehler wäre, gegen den diese Iteration gebaut ist." Diesen Fall gibt es seit diesem PR nicht mehr: Bei null getroffenen Zeilen liest `updateMaster` seither nach ([features/calendar/recurrence.ts](../features/calendar/recurrence.ts)) und wirft entweder `EventConflictError` **mit** der fremden Zeile oder, wenn auch die Nachlese leer bleibt, `EventNotFoundError` — nie mehr `EventConflictError(null)`. ADR-031 bleibt unverändert stehen; diese eine Aussage dort ist ab 2026-09-18 überholt. **Der Rest von Decision 12 gilt unverändert weiter:** Ein leerer Feldvergleich heißt weiterhin „kein Dialog, Schreibvorgang läuft mit frischer Basis durch", und ein Dialog **ohne** Vergleichszeilen bleibt möglich — aber nicht mehr aus dem in Decision 12 genannten Grund (dem CAS-Fall ohne fremde Fassung), sondern aus den dort nicht behandelten: `theirs === null`, wenn die angeforderte Occurrence außerhalb des Suchfensters lag (Decision 6 unten), einer beim Eintreffen des Konflikts noch nicht hydrierten Basis (`baseOccurrence`/`baseTask === null`), oder einem erschöpften Auto-Retry-Limit, ohne dass ein Feld abweicht. Der leere Dialog verschwindet nicht; nur der eine Auslöser, bei dem die fremde Fassung unbekannt war, tut es.

Setzt den zweiten Spiegelstrich von [Roadmap 2.2](./roadmap.md#22-die-zwei-löcher-in-der-conflict-detection--m) um — der erste, die fehlende Wiederholungsregel im Vergleich, bleibt offen und ist für ADR-038 (PR 3) vorgesehen. Zweiter von drei ADRs aus Block 2 der Roadmap (036 Aufgaben-Schreibpfad → 037 dieser → 038 die Regel im Vergleich). Spec: [2026-09-17-tasks-delete-path-conflict-gaps-design.md](./superpowers/specs/2026-09-17-tasks-delete-path-conflict-gaps-design.md) §4 („PR 2 — Der CAS-Fall trägt seine Zeile").
...
- Der zeilenlose Vergleichs-Dialog verschwindet nicht — `theirs === null` (Occurrence außerhalb des Suchfensters), eine noch nicht hydrierte Basis (`baseOccurrence`/`baseTask === null`) und ein erschöpftes Auto-Retry-Limit bleiben alle drei möglich. Was sich ändert: Er hat jetzt in jedem Fall eine frische Basis-Version, weil `row` nicht mehr fehlen kann — „Deine Fassung speichern" kann nicht mehr endlos gegen dieselbe veraltete Version laufen. Was verschwindet, ist der eine Auslöser, bei dem die fremde Fassung unbekannt war.
