...
14. **`conflict.*` statt `cal.*`.** Ein Dialog für zwei Features — Kalender und Aufgaben benutzen dieselben acht Schlüssel —, also ein familienweiter Namespace, wie `sync.*` in ADR-030 Decision 15 eine Zeile für alle trägt. Feldnamen kommen bewusst **nicht** dazu: Der Dialog beschriftet seine Zeilen mit den vorhandenen `cal.edit.field*`- und `hw.*`-Schlüsseln, und die beiden Fehler-Keys bleiben feature-lokal (`cal.error.conflict`, `hw.error.conflict`), weil sie in feature-eigenen Toasts landen. „Deine Fassung", nicht „Ihre Fassung" — Letzteres liest sich im Deutschen als Höflichkeitsform und verstieße gegen die Du-Regel.
