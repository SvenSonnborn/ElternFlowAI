...
7. **ccache soll den From-Source-Build tragbar machen — die Wirkung ist noch nicht belegt.** `RCT_USE_PREBUILT_RNCORE=0` heißt React Native aus dem Quellcode; `USE_CCACHE=1` schaltet RNs eigene ccache-Unterstützung ein (das Expo-Podfile reicht die Variable über `ccache_enabled?` an `react_native_post_install` durch, es ist kein Fremdkörper). `CCACHE_MAXSIZE` steht auf 1 G, weil der Cache-Key SHA-suffigiert ist: bei 2 G hätten wenige Pushes das 10-GB-Budget des Repos gefüllt und per LRU die Gradle- und CocoaPods-Caches verdrängt — der Cache hätte sich selbst aufgefressen. **Die ursprüngliche Fassung dieser Decision behauptete „drückt das in den ~15-Minuten-Bereich". Das war eine ungeprüfte Schätzung, und die ersten beiden Läufe haben sie widerlegt** — siehe die Messwerte in den Consequences.
...
- **Gemessene Laufzeiten (PR #92, die ersten beiden Läufe):** Metro-Bundle 1m49s → 1m46s, Android 17m47s → 19m10s, iOS 37m42s → 36m30s. Der zweite Lauf hätte warme Caches haben müssen und war praktisch identisch. **Die Caches griffen nicht**, aus zwei verschiedenen Gründen:
  - **iOS: `CCACHE_DIR` erreichte den Compiler nie.** Der Wert stand nur im `env:`-Block des Jobs, und Xcode reicht Shell-Umgebungsvariablen nicht an Compiler-Tasks durch — es baut eine kontrollierte Umgebung aus den Build-Settings. Genau darüber kommt RNs `CCACHE_BINARY` bei seinem Wrapper an (`react-native/scripts/xcode/ccache-clang.sh`, gesetzt in `cocoapods/utils.rb`). ccache lief also korrekt (`[Ccache]: Setting CC, LD, CXX & LDPLUSPLUS build settings` im Log), schrieb aber in sein Standardverzeichnis, während der Workflow `/tmp/ccache` cachte und inspizierte — daher die irreführende Meldung „Cache size (GB): 0.0 / 1.0". Fix: `CCACHE_DIR` und `CCACHE_MAXSIZE` werden zusätzlich als Build-Settings an `xcodebuild` übergeben, und der Statistik-Step druckt jetzt die Größe **beider** Kandidatenverzeichnisse, damit ein Fehlschlag nicht wieder als leerer Cache getarnt ist.
  - **Android: es gab nie etwas Wiederverwendbares zu cachen.** `~/.gradle/caches` enthält heruntergeladene Dependencies, nicht kompilierte Ausgaben — Gradles Build-Cache ist per Default aus. Der teure Teil liegt in `android/build`, das `prebuild --clean` bei jedem Lauf löscht. Fix: `--build-cache` am `gradlew`-Aufruf; das lokale Cache-Verzeichnis liegt unter `~/.gradle/caches/build-cache-1` und ist vom bestehenden Cache-Step bereits abgedeckt. Die Erwartung bleibt gedämpft: `externalNativeBuild`-Tasks (CMake/NDK) sind nicht cachebar, und genau die dürften den Großteil ausmachen.

  **Beide Fixes sind zum Zeitpunkt dieser ADR noch nicht gemessen.** Wer die nächste Zahl sieht, trägt sie hier nach — und wenn iOS auch dann bei ~36 Minuten bleibt, ist das die Antwort auf Decision 1 und der Pfad-Filter gehört ernsthaft auf den Tisch.
