...
4. **Der Route-Parameter `occ` trägt den Schlüssel.** Ein Deep-Link von vor diesem PR trägt noch ein aufgelöstes Datum — gebildet in der **Gerätezone** des Erstellers, nicht in `row.timezone`. Für eine unverschobene Occurrence greift `useEvent`s `expanded.find(...) ?? expanded[0]`-Fallback trotzdem (beide Felder sind dort gleich). Bei einer verschobenen Occurrence ist der Fall nicht so harmlos, wie „`find` findet nichts, fällt zurück" klingt: Weicht die Gerätezone des Link-Öffners von `events.timezone` so ab, dass die Tagesgrenze fällt (ein täglicher Berliner Termin um 00:30, geöffnet in `America/New_York`), trägt der alte Link z. B. `"2026-06-01"`, während der `occurrenceKey` derselben Occurrence `"2026-06-02"` lautet. `find` schlägt dann **nicht** fehl — es trifft die **Nachbar-Occurrence**, deren Schlüssel zufällig mit dem alten Datum übereinstimmt. Der Nutzer landet auf einem plausibel aussehenden, falschen Termin, und ein anschließendes „Nur diesen" schreibt oder löscht die Exception-Zeile des Nachbarn statt der gemeinten. Nur bei einem echten Nicht-Treffer greift `expanded[0]`. Der Vektor ist schmal (keine Push- oder Mail-Deep-Links, nur Browser-Tab oder Lesezeichen) — aber „kein Datenverlust, nur ein falsches Ziel" gilt seitdem nicht mehr uneingeschränkt.
