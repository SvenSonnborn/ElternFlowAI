...
- **Supersession** der letzten Aussage in ADR-031 Decision 12: „Beim CAS-Fall ohne fremde Fassung (`EventConflictError(null)`) gibt es nichts zu vergleichen; dort erscheint der Dialog **ohne** Zeilen, weil die Alternative — stilles Durchwinken — genau der Fehler wäre, gegen den diese Iteration gebaut ist." Diesen Fall gibt es seit diesem PR nicht mehr: Bei null getroffenen Zeilen liest `updateMaster` seither nach ([features/calendar/recurrence.ts](../features/calendar/recurrence.ts)) und wirft entweder `EventConflictError` **mit** der fremden Zeile oder, wenn auch die Nachlese leer bleibt, `EventNotFoundError` — nie mehr `EventConflictError(null)`. ADR-031 bleibt unverändert stehen; diese eine Aussage dort ist ab 2026-09-18 überholt. **Der Rest von Decision 12 gilt unverändert weiter:** Ein leerer Feldvergleich heißt weiterhin „kein Dialog, Schreibvorgang läuft mit frischer Basis durch", und ein Dialog **ohne** Vergleichszeilen bleibt möglich — nur aus dem anderen der beiden ursprünglich genannten Gründe: `theirs === null`, wenn die angeforderte Occurrence außerhalb des Suchfensters lag (Decision 6 unten). Der leere Dialog verschwindet nicht; nur einer seiner beiden Auslöser tut es.
