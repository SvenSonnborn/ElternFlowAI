...
| `all`     | schreibt alle fünf Felder auf den Master                                     | `title`, `location` **und** `description` ändern sich; jede Occurrence bekommt die **neue Tageszeit** — und behält ihr Datum **nur, solange der Datumsanteil unverändert bleibt** (siehe unten) | alle fünf Felder patchen; Tageszeit und Dauer aus `changes` ableiten und pro Occurrence-Datum neu verankern |
...
**Korrektur nach der Schluss-Review: „behält ihr Datum“ gilt nicht unbedingt.** Der Server schreibt `changes.start_at` per `updateMaster` in `events.start_at` — und das ist `dtstart` der RRULE. Ändert der Nutzer bei `all`/`forward` den **Datumsanteil**, wandert die ganze Serie mit; das Overlay kann aber nur die Tageszeit neu verankern und lässt jedes Occurrence-Datum stehen. Nachgerechnet an einer monatlichen Serie am 5., verschoben auf den 7.10.: Das Overlay zeigt `[09-05, 10-05, 11-05]`, der Refetch liefert `[10-07, 11-07, 12-07]`. Das ist nicht die Näherung aus Decision 3 — ein sichtbares Zurechtrücken —, sondern die Anzeige der **Nicht-Änderung genau der Eigenschaft, die der Nutzer geändert hat**: Im Kalender steht alles unverändert, der Nutzer hält das Speichern für fehlgeschlagen, und eine Sekunde später springt die ganze Serie.

Konsequenz: **`all`/`forward` mit geändertem Datum bekommt gar keinen optimistischen Eintrag** — dieselbe Enthaltsamkeit, die `useUpdateEvent` bei `vars.recurrence` übt. Entschieden wird das in `canApplyOptimistically` ([optimisticEvents.ts](../../../features/calendar/optimisticEvents.ts)), einer reinen Funktion neben `applyOptimisticChanges`, damit die Fallunterscheidung ohne React testbar ist.
